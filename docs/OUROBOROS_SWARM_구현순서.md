...
- [x] 시작점·방향 스냅샷
- [x] 0.2초 예고
- [x] 길이 7, 폭 0.35의 `EnemyHurtbox` Collider 비할당 교차 판정
- [x] 고유 적 1회 피해 12
- [x] 예고 중 세그먼트 제거 취소
...
- 상태: 완료
- 최근 갱신: 2026-07-19
- 완료: `OSLaserBodyRole`의 적 중심점 `IsInsideBeam` 판정을 `EnemyHurtbox` 레이어 전용 `Physics2D.OverlapBox`로 교체했다. 예고 스냅샷과 동일한 시작점·방향·길이 7·폭 0.35 사각형을 사용하며 256칸 고정 Collider/적/Runtime ID 버퍼로 조회 결과를 모아 프레임별 컬렉션과 GC 할당을 만들지 않는다. 같은 적의 여러 Collider는 Runtime ID로 중복 제거한 뒤 등록·생존 상태를 확인해 한 빔당 12 피해를 1회만 적용하고, 서로 다른 범위 내 적은 모두 피해 처리한다. `20_Game.unity`, Step 11 재생성 경로와 `Ouroboros/Setup/Apply Step 15.4 Laser Collider Coverage` 메뉴에 `EnemyHurtbox` 마스크를 고정했으며 Step 전용 WebGL 빌드 메뉴를 추가했다.
- 남음: 없음. Step 16 성능 최적화·WebGL 프로파일 단계로 진행할 수 있다.
...
- Step 15.4 레이저 Collider 범위 판정과 전체 런 확정
