...
- 상태: 완료
- 최근 갱신: 2026-07-20
- 완료: `Assets/Ouroboros/Scripts/Core/OSPathSampleRingBuffer.cs`에 누적 거리 샘플·고정 배열 원형 버퍼·wrap-around·경계 보간·가상 직선 경로·필요 거리 보존과 64칸 용량 계산을 구현했다. `OSBodyChain.cs`는 머리부터 꼬리 방향 단일 순회로 목표 위치를 계산하고, 안정 ID·체인 인덱스·역할 이벤트와 2/20/40/64 디버그 생성을 제공한다. Step 15.14에서 절단 몸통 회수 묶음마다 수집 순간의 기존 마지막 꼬리 누적 경로 좌표를 고정 배열에 저장하고, 모든 신규 View를 그 위치에 겹친 뒤 머리 이동 거리 1칸마다 앞쪽부터 정상 간격에 합류하도록 확장했다. `PF_BodySegment.prefab`과 Runtime/View 슬롯 64개를 세션 시작 전에 사전 생성하며, `PlayerBodyHurtbox` Trigger는 자기 몸·머리·월드 장애물과 물리 충돌하지 않는다. `20_Game.unity`에는 20칸 디버그 표본과 Step 05 재적용 메뉴를 연결했다.
...
- 상태: 완료
- 최근 갱신: 2026-07-19
- 완료: `OSWaveDirector`의 10:00 이벤트를 `OSBossEncounterController`에 연결해 `boss_swarm_core`를 전용 1칸 풀에서 결정적 위치에 1회 생성한다. `OSBossController`는 HP 6,000, 보호막 600, Step 15.17 기준 150초 제한, HP 70%/35% 페이즈, 0.6초 부채꼴 5/7발, 0.8초 추적·분열체 6/8마리 소환, 0.8초 충돌 안전 흡인, 0.5초 보호막 패턴을 4.0/3.4/2.8초 간격으로 실행하고 한 시점의 고위험 예고를 1개로 제한한다. 제어는 이동만 제한하고 캐스팅은 유지하며, 일반 적 180 상한은 보스 전용 슬롯과 분리한다. `OSBossEncounterController`가 선택 중 패턴·제한 시간을 정지하고 보스 사망을 플레이어 피해·폭발 처리 뒤에 확정해 동시 사망은 플레이어 실패로 처리하며, 처치·150초 시간 초과를 서로 다른 세션 결과로 확정한다. `OSRunSummaryController`와 `OSResultPanel`은 결과 종류, 생존 시간, 최대/최종·획득/절단/폭발 소비 몸통, 총/폭발/정예 처치, 받은 머리 피해, 역할별 최대/최종 수, 레벨·업그레이드, 런 시드·데이터 버전을 고정하고 재시작·메인 메뉴를 제공한다. 보스 HP/보호막/남은 시간·페이즈·패턴 HUD, `PF_Boss_SwarmCore`, 재적용 메뉴 `Ouroboros/Setup/Apply Step 14 Boss And Results`, Step 14 WebGL 빌드 메뉴를 `20_Game.unity`에 연결했다.
...
- 상태: 완료
- 최근 갱신: 2026-07-20
- 완료: `OSCombatHudPresenter`가 HP·무적, 몸통과 `[O][>][=][+]` 역할 수, 조각, 레벨/경험치, 세션 시간·활성 적, 실드, 폭발 예상 소비·실제 예약, 정예·보스 경고를 이벤트 기반 dirty 표시 모델로 모아 프레임 말 최대 1회 갱신한다. HUD는 몸통 64를 최대치로 표시하지 않으며 Body 4에서 `x2 -> BODY 2`처럼 폭발 뒤 잔존량을 미리 보여 준다. Step 15.14에서 갱신되지 않던 구형 정적 `HP 100/100` 텍스트를 추가로 숨기고 통합 HUD의 플레이어 `CORE` 문구를 `OSPlayerHealth.HealthChanged` 기반 단일 `HP` 표기로 정리했다. `OSTutorialProgress`/`OSTutorialPresenter`는 첫 세션에만 이동 1초→자동 공격/첫 처치→첫 조각/역할 선택→Body 4 폭발→첫 절단 차이를 조건 기반 한 문장으로 안내하고 `Time.timeScale == 0`과 독립적으로 진행한다. `OSCombatFeedbackPresenter`는 머리 피해와 절단을 구분하고 절단 위치·꼬리 방향·상실 역할 수를 한 피드백으로 축약한다. 역할/레벨 카드 포커스는 unscaled 시간으로 동작하고 역할 카드에 고유 문양·현재 수·주요 수치·꼬리 추가 문구를 표시한다. 머리 코어 220, 적/보스 위험 예고 200, 레이저 198, 폭발 예약 190, 절단 185, 제어 상태 182, 실드 175의 Sorting 우선순위와 제어 잔여량 원호를 씬·적 프리팹에 적용했다. 결과 패널 대비·간격을 마감하고 기존 분산 HUD의 중복 텍스트 렌더만 숨겼으며 규칙 Presenter 생명주기는 유지했다. 재적용 메뉴 `Ouroboros/Setup/Apply Step 15 Readability`와 WebGL 빌드 메뉴를 추가했다.
- 남음: 없음. 사운드·장식 VFX와 효과음 동시 재생 제한 검증은 재배치된 Step 18 범위다.
...
- [x] 연속 절단·폭발 예약 변경 시 UI 피드백 중복 없음(효과음은 Step 18에서 검증)
...
Step 17 성능 측정 전에 전장 크기와 `WorldBlocker`의 공통 이동·투사체 계약을 확정한다.
...
- 남음: 없음. Step 16 사용자 플레이 피드백 반복 개선 단계로 진행할 수 있다.
...
- 남음: 없음. Step 전용 커밋·원격 푸시 뒤 Step 16 사용자 플레이 피드백 반복 개선으로 진행할 수 있다.
...
## 16.14 꼬리 회수 전개·HP HUD 정리 — Step 15.14

### 목표

여러 절단 몸통을 한 번에 주웠을 때 체인이 과거 경로 뒤로 즉시 길어지지 않게 하고, 플레이어 체력을 갱신되는 단일 HP 표기로 정리한다.

### 구현 현황

- 상태: 완료
- 최근 갱신: 2026-07-20
- 완료: `OSBodyChain.AppendReclaimedSegments`가 회수 시작 시 기존 마지막 꼬리의 누적 경로 좌표를 한 번만 계산하고 한 묶음의 모든 신규 세그먼트에 같은 기준점을 적용한다. 회수 View는 즉시 그 위치에 겹치며, 각 세그먼트의 정상 목표 경로가 기준점을 따라잡을 만큼 머리가 이동했을 때 머리에 가까운 순서로 고정을 해제한다. 역할·수량·안정 ID·기술 가드 부분 회수는 유지하고 기준점/활성 상태는 몸통 풀과 함께 사전 할당한 배열에 저장한다. `OSCombatHudPresenter`의 플레이어 `CORE` 문구를 `HP`로 바꾸고, 갱신 코드가 없던 `Canvas/CombatHUD/HP` 정적 텍스트를 비활성화했으며 Step 15 재적용 경로도 같은 중복 제거를 수행한다. `Ouroboros/Setup/Apply Step 15.14 Tail Reclaim And HP HUD`와 전용 WebGL 빌드 메뉴를 추가했다.
- 남음: 없음. Step 전용 커밋·원격 푸시 뒤 Step 16 사용자 플레이 피드백 반복 개선으로 진행할 수 있다.
- 검증: Unity 6000.5.1f1 컴파일 오류 0, 최종 Console Error/Exception 0. `OSBodyGrowthPlayModeTests`+`OSStep15ScenePlayModeTests` 집중 16/16 통과(job `aa85822fe81248d4800eacf72a5a9655`)로 3칸 회수 직후 기존 꼬리 한 점 겹침, 이동 거리 0.55마다 순차 합류, 최종 간격 복원, 정적 HP 비활성, 머리 피해 100→92의 통합 HP 반영을 확인했다. 전체 `Ouroboros.Tests.EditMode` 62/62 통과(job `5ce4509784ef44b8b2ff8c018e3fff75`), 전체 `Ouroboros.Tests.PlayMode` 117/117 통과(job `a3e1117f90c54af69adc46d744c9cabe`). 승인 `WebGL Development` Build Profile의 `Builds/Step15_14/WebGL/` 빌드 성공(135,673,756 bytes, errors 0, warnings 5; TMP IL2CPP 대형 메서드 분리 정보 3건 포함). `Tools/Serve-WebGL.ps1`로 `http://127.0.0.1:8134/`을 실행해 index/WASM HTTP 200, WASM MIME `application/wasm`, 960×540 Canvas, MainMenu→Attack/Laser→Combat과 플레이어 `HP 100/100` 단일 표기를 확인했다. 유효 머리 피해 100→92 갱신은 자동 회귀로 보장했고 브라우저 error 레벨은 0건이며 WebGL 고유 URP FSR 비지원 warning 1건만 남았다. Windows 빌드는 실행하지 않았다.

### 프로그래머 체크

- [x] 회수 묶음의 공통 마지막 꼬리 경로 기준점
- [x] 머리 이동 거리 기반 앞쪽부터 순차 전개
- [x] 역할·수량·안정 ID·부분 회수 불변
- [x] 사전 할당 배열 사용과 회수 시 컬렉션 생성 없음
- [x] 구형 정적 HP 비활성 및 통합 HUD 단일 HP 표기
- [x] EditMode/PlayMode 전체 회귀
- [x] WebGL HTTP·Canvas·HP 단일 표기·Console 검증

### 완료 기준

- 한 픽업에서 여러 절단 몸통을 회수해도 신규 몸통 전량이 수집 순간의 기존 마지막 꼬리 위치에 겹친다.
- 정지 중에는 겹침을 유지하고, 이동하면 세그먼트 간격만큼 진행할 때마다 앞쪽 회수 몸통부터 연속적으로 정상 경로에 합류한다.
- 화면에는 플레이어 HP가 하나만 보이며 유효 머리 피해와 회복 시 `OSPlayerHealth` 값과 일치한다.
- WebGL 전투가 로드되고 브라우저 Error/Exception이 없다.
...
- 남음: 없음. Step 16 사용자 플레이 피드백 반복 개선 착수 가능.
...
- 남음: 없음. Step 전용 커밋·원격 푸시 뒤 Step 16 사용자 플레이 피드백 반복 개선 착수 가능.
...
## 16.17 전투 명료성 마감 — Step 15.17

### 목표

보스 제한 시간을 2분 30초로 조정하고, 공용 풀을 쓰는 일반 픽업을 색상과 실루엣으로 구분하며, 레이저 표시 굵기와 실제 피격 굵기를 동일한 값으로 고정한다.

### 구현 현황

- 상태: 완료
- 최근 갱신: 2026-07-20
- 완료: `OSBossEncounterController`의 기본값과 `20_Game.unity`, Step 14 재적용 경로를 150초로 맞추고 `OSResultPanel`의 시간 초과 문구를 `150s TIMEOUT`으로 갱신했다. 선택 중 제한 시간 정지와 플레이어 피해 후 보스 결과 판정 순서는 유지한다. `OSPickup`은 공용 256개 풀 안에서 몸통 조각을 청록 `Body_Control` 마름모, 경험치를 금색 `Projectile` 원형, 회복을 초록 `Pickup` 십자로 재설정하며 절단 몸통 역할 비주얼은 기존 규칙을 유지한다. `OSLaserBodyRole`은 예고·발사 때 `LineRenderer.widthMultiplier`를 1로 고정하고 시작·끝 폭에 판정 스냅샷의 `Width`를 그대로 사용해 `Physics2D.OverlapBox` 전체 폭과 표시 굵기를 일치시켰다. 재적용 메뉴 `Ouroboros/Setup/Apply Step 15.17 Combat Clarity`와 WebGL 빌드 메뉴를 추가했다.
- 남음: 없음. Step 전용 커밋·원격 푸시 뒤 Step 16 사용자 플레이 피드백 반복 개선 착수 가능.
- 검증: Unity 6000.5.1f1 집중 `OSBodyRolesPlayModeTests`, `OSStep14ScenePlayModeTests`, `OSStep15PickupScatterPlayModeTests` 28/28 통과(job `9d6ea5ff85134d2687aa43129e035d0c`)로 레이저 표시/판정 폭 일치, 149.9초 생존·150초 시간 초과 경계, `150s TIMEOUT`, 일반 픽업 3종의 고유 스프라이트·색상을 확인했다. 전체 EditMode 63/63(job `15f32c1b01eb4fa28f4d946ecf7c60a1`)와 PlayMode 118/118(job `3b5ab418e6c649d5b46a43b887e49c97`)이 통과했다. `Builds/Step15_17/WebGL/` Development 빌드는 오류 0·경고 5·135,678,602바이트로 성공했고 Windows 빌드는 실행하지 않았다. HTTP에서 `index.html`과 `WebGL.wasm`이 200, wasm MIME `application/wasm`, Canvas 960×540을 확인했다. 브라우저 전투 화면에서 청록 마름모 몸통 조각과 금색 원형 경험치의 구분을 확인했으며 Error 0, 기존 URP FSR 미지원 경고 1건만 남았다.

### 프로그래머 체크

- [x] 보스 제한 시간 기본값·씬·재적용 경로 150초 통일
- [x] 선택 중 제한 시간 정지와 150초 경계 회귀
- [x] 결과 화면 `150s TIMEOUT` 표시
- [x] 몸통 조각·경험치·회복의 고유 스프라이트와 색상
- [x] 절단 몸통 역할 비주얼과 풀 재대여 타입 복구 유지
- [x] 레이저 `LineRenderer` 전체 폭과 `OverlapBox` 판정 폭 일치
- [x] Step 15.17 전용 재적용·WebGL 빌드 경로
- [x] 집중 테스트와 전체 EditMode/PlayMode 회귀
- [x] WebGL HTTP·Canvas·픽업 렌더·Console 검증

### 완료 기준

- 보스는 선택 시간을 제외한 전투 시간 150초에서 시간 초과되고 결과 문구도 같은 값을 표시한다.
- 일반 픽업 3종은 색각 정보가 없어도 실루엣으로 구분되며 공용 풀 재대여 뒤에도 현재 타입 표시가 맞다.
- 레이저 예고·발사선의 전체 굵기가 같은 스냅샷으로 수행되는 실제 `OverlapBox` 피격 폭과 일치한다.
...
## 17. 사용자 플레이 피드백 반복 개선 — Step 16

### 목표

Step 15.x에서 이어 온 방식으로 최신 WebGL 빌드를 사용자가 직접 플레이해 확인한 게임성 피드백을 작은 `Step 16.x` 단위로 나누고, 합의된 체감과 규칙에 도달할 때까지 구현·검증을 반복한다.

### 구현 현황

- 상태: 진행 중
- 최근 갱신: 2026-07-24
- 완료: Step 15.19 검증·커밋·원격 푸시까지 끝난 빌드를 반복 개선 기준선으로 확정하고, 사용자 결정에 따라 기존 성능 단계를 Step 17로 미룬 뒤 Step 16을 사용자 플레이 피드백 반복 구간으로 재정의했다. 첫 반복인 Step 16.1 Bomb은 전용 커밋 `e2de571`과 원격 푸시까지 완료했으며, 후속 플레이 피드백인 Step 16.2 Bomb 리듬·밀도 개선의 규칙·구현·자동 회귀·WebGL 실행 검증을 완료했다.
- 남음: Step 16.2 전용 커밋·원격 푸시 뒤 사용자 플레이 확인을 받고 다음 `Step 16.x` 피드백을 같은 절차로 반복한다.
- 검증: Step 16.1 최종 전체 EditMode 72/72(job `28fc9097e11747c9ac79ee44e04164d1`)와 PlayMode 128/128(job `47e093401a064132809d658b9e4e76df`) 통과. Step 16.2 최종 전체 EditMode 74/74(job `85bf628527fa4468a952acff71fb2a33`)와 PlayMode 129/129(job `93b827e3d6b242ba8eb36464fc6a123e`) 통과. WebGL 세부 결과는 해당 구현 현황에 기록한다.

## 17.1 Bomb 원형 폭발 — Step 16.1

### 관찰과 기대 체감

- 관찰: Space 몸통 수축 대시는 몸통을 보존하는 이동기에 집중돼 있어, 충분히 성장한 몸통을 직접 공격 자원으로 전환하는 선택지가 없다.
- 기대 체감: B를 누르면 몸통 일부를 희생해 머리가 진행 방향 앞의 원을 1초 동안 크게 그리고, 몸통이 그 경로를 따라 적을 감싼 뒤 한 번에 폭발하며 원을 따라 머리로 모여야 한다.
- 안전 조건: 몸통 10칸 미만, 재사용 대기 중, 또는 머리 Solid가 원 전체를 돌 수 없는 장애물·월드 경계 배치에서는 아무 비용 없이 사용이 거부돼야 한다.

### 확정 범위

- `Player/Bomb` B 입력은 `Combat`에서만 유효하고, 활성 몸통 10칸 이상일 때 현재 수의 10%를 소수점 버림해 꼬리부터 소비한다.
- 원 둘레는 소비 후 남은 몸통 수 × 세그먼트 간격이며, 원 중심은 발동 지점에서 머리 스냅샷 방향으로 반지름만큼 앞에 둔다.
- 소비 전 몸통의 좌·우 개수와 횡방향 편향을 비교해 몸통이 많은 쪽의 반대로 회전하며 완전 동률은 오른쪽이다.
- 머리는 1초 동안 원을 한 바퀴 그리고 몸통은 기존 누적 거리 경로를 따른다. 폭발 뒤 0.5초 동안 직선이 아니라 기록된 원 경로를 따라 머리로 집결하고, 이후 평상 이동 거리로 자연 전개한다.
- 발동부터 집결 완료까지 머리 피해와 몸통 절단에 무적이며 유효하지 않은 공격은 Shield를 소비하지 않는다.
- 폭발은 원 내부 또는 경계와 겹친 등록 적의 `EnemyHurtbox`를 찾아 Runtime ID마다 고정 피해 1회를 준다. 기본 피해는 가설 100이다.
- 기본 재사용 대기는 발동 시점부터 10초다. `bomb_damage`는 단계당 피해 +20%·최대 3단계, `bomb_cooldown`은 단계당 -1초·최대 3단계·최종 하한 5초다.

### 구현 현황

- 상태: 완료
- 최근 갱신: 2026-07-24
- 완료: `OSBombMath`, `OSBombController`, `OSBombView`를 추가하고 입력·세션 상태·머리 원 선검사/주행·몸통 소비/경로 집결·머리/몸통 능력 무적·고유 적 폭발 피해를 연결했다. `OSBodyBalanceData`와 세션 복사본에 Bomb 설정을 추가하고 업그레이드 카탈로그를 17종으로 확장했으며 HUD·결과 통계·피드백 키·씬 VFX를 연결했다. 재적용 메뉴 `Ouroboros/Setup/Apply Step 16.1 Bomb`과 WebGL 빌드 메뉴를 추가했다.
- 남음: 없음. 전용 커밋 `e2de571`과 원격 푸시 완료 뒤 Step 16.2 사용자 플레이 피드백 반복으로 진행했다.
- 검증: Unity 6000.5.1f1 강제 컴파일·도메인 재로드 후 최종 Console Error/Exception 0. 집중 EditMode 16/16(job `6ab7fd0c355e4c76a363bc244ff156d2`), 수정 확인 PlayMode 2/2(job `71ce36b98a774d548a43859b71647886`), Bomb·입력·업그레이드 집중 PlayMode 17/17(job `dde52093ca094587b693254e2c72ddd1`) 통과. 비활성화 취소 회귀 추가 뒤 최종 전체 EditMode 72/72(job `28fc9097e11747c9ac79ee44e04164d1`)와 PlayMode 128/128(job `47e093401a064132809d658b9e4e76df`) 통과. 승인 `WebGL Development` Profile의 `Builds/Step16_1/WebGL/` 최종 빌드는 errors 0·warnings 3·135,776,850 bytes로 성공했고 Windows 빌드는 실행하지 않았다. HTTP에서 `index.html`과 `WebGL.wasm` 200, WASM MIME `application/wasm`, WASM 120,968,817 bytes와 Canvas 960×540을 확인했다. MainMenu→Shield 역할 2회→Combat에서 `BOMB NEEDS 10 BODY [B]` HUD와 몸통 2칸 B 입력의 몸통·상태 무변경 거부를 확인했다. 브라우저 Error는 0건이고 기존 URP FSR 비지원 Warning 1건만 기록됐다. 몸통 10칸 이상의 전체 Bomb 원 주행·폭발·집결은 PlayMode 자동 회귀로 확인했다.

### 체크리스트

- [x] B 입력이 Player Map의 Bomb Action으로만 전달되고 선택·결과 상태에서 예약 실행되지 않음
- [x] 몸통 9칸 거부, 10/19/20/64칸의 `floor(N×0.10)` 소비 경계와 꼬리 역할 제거
- [x] 소비 후 몸통 길이로 반지름 계산, 좌우 몸통 반대 회전과 완전 동률 오른쪽 결정
- [x] 전체 원의 `WorldBlocker`·월드 경계 선검사 실패 시 몸통·재사용·무적 무변경
- [x] 1초 원 주행과 정상 몸통 추종, 0.5초 원 경로 집결, 정지 뭉침과 이동 거리 자연 전개
- [x] 전 구간 머리·몸통 무적과 Shield 무소비
- [x] 원 내부·경계 Collider 겹침, 같은 적 복수 Collider Runtime ID 중복 제거, 고정 피해 1회
- [x] 기본 피해 100·재사용 10초와 피해 160·재사용 7초 최대 단계 회귀
- [x] HUD 준비/거부/재사용/그리기/집결과 결과의 사용·소비·명중 통계
- [x] 전체 EditMode 72/72, PlayMode 128/128, 최종 Console Error/Exception 0
- [x] WebGL Development 빌드와 HTTP Canvas·브라우저 Console 확인

### 완료 기준

- B 입력 1회로 선검사·비용·원 주행·폭발·원 경로 집결이 순서대로 실행되고 실패 입력은 부작용이 없다.
- 몸통 수와 배치에 따라 원 크기와 회전 방향이 결정적이며 폭발 피해·재사용 업그레이드가 다른 Bomb 속성을 바꾸지 않는다.
- 기존 이동·Space 대시·역할·절단·선택·사망·웨이브·보스 회귀와 WebGL 실행이 유지된다.

### 반복 운영 규칙

- 사용자 피드백 한 묶음을 독립적으로 검증 가능한 가장 작은 `Step 16.x`로 등록한다. 서로 분리 가능한 피드백을 한 Step에 합치지 않는다.
- 착수 전에 관찰된 문제, 재현 조건, 기대 체감, 변경 범위와 완료 기준을 구현순서 문서에 먼저 기록한다.
- 피드백이 게임 규칙·확정 수치·데이터 계약·클래스 책임을 바꾸면 설계서의 관련 본문과 영향을 받는 기존 Step도 같은 변경 묶음에서 갱신한다.
- 각 `Step 16.x`는 코드·에셋·설정 변경, Unity 컴파일 및 Console 확인, 위험에 맞는 집중/전체 테스트, WebGL Development 빌드와 HTTP 브라우저 수동 확인, 구현 현황 갱신을 한 묶음으로 완료한다.
- 한 `Step 16.x`의 완료 기준과 검증이 모두 통과하면 전용 커밋을 만들고 원격에 푸시한 뒤 다음 피드백에 착수한다.
- 단순 선호와 재현 가능한 결함을 구분하되, 사용자 체감 목표를 임의의 내부 지표로 대체하지 않는다. 합의되지 않은 규칙 변경은 구현하지 않는다.
- 반복 중 발견된 배포 차단 성능 문제는 해당 피드백 범위에서 고치고 검증할 수 있지만, 전체 피크 시나리오와 성능 예산 확정은 Step 17에서 수행한다.

### 반복 순서

1. 최신 원격 반영 WebGL 빌드로 사용자 플레이 피드백을 수집한다.
2. 피드백을 재현하고 버그·규칙 변경·밸런스 가설·판독성 개선으로 분류한다.
3. 가장 작은 `Step 16.x` 범위, 관련 기준 문서, 자동/수동 수용 기준을 확정한다.
4. 구현 후 집중 회귀와 전체 회귀를 통과시킨다.
5. WebGL 빌드를 HTTP로 실행해 실제 체감과 Console을 확인한다.
6. 사용자 확인 결과에 따라 다음 `Step 16.x`를 열거나 Step 16을 종료한다.

### 완료 기준

- 사용자가 합의된 플레이 피드백 반영 완료와 게임성 기준선 확정을 확인한다.
- 열려 있는 P0 게임성 피드백이 없고, 미반영 항목은 P1/P2 백로그 또는 명시적 보류로 분리돼 있다.
- 마지막 반복 빌드에서 전체 EditMode·PlayMode, WebGL Development 빌드, HTTP Canvas·Console과 해당 피드백의 수동 수용 기준이 통과한다.
- 변경된 게임 규칙·수치·데이터 계약과 기존 Step 구현 현황이 실제 코드·에셋에 맞게 갱신돼 있다.
- 마지막 `Step 16.x` 전용 커밋과 원격 푸시가 성공한 뒤 Step 17 성능 최적화·WebGL로 진행한다.

---

## 17.2 Bomb 리듬·전장 밀도 개선 — Step 16.2

### 관찰과 기대 체감

- 관찰: Step 16.1의 머리 원 주행은 1초 내내 같은 각속도라 움직임이 기계적으로 보이고, 실제 발동 전에 원 크기와 장애물 관계를 확인할 수 없다.
- 관찰: 성장 뒤에도 원 크기와 폭발 피해의 증가 체감이 작고, 장애물 때문에 원을 만들 수 없으면 입력이 거부돼 공격 기회가 끊긴다. 현재 적 밀도도 커진 Bomb의 범위와 화력을 충분히 시험하기에는 낮다.
- 기대 체감: B를 누르는 동안 이동에 맞춰 완성 원을 실시간으로 조준하고, 놓으면 초반·후반은 부드럽고 중간은 빠른 리듬으로 더 큰 원을 그린다. 벽과 경계를 통과하더라도 원점으로 돌아오며, 긴 몸통일수록 폭발 피해가 분명히 커지고 더 많은 적을 한 번에 감싼다.

### 확정 범위

- `B` 홀드 중 현재 머리 위치·진행 방향·몸통 좌우 배치와 소비 후 길이를 사용한 완성 원을 표시하고, 릴리스 때만 발동한다. Player Map이 꺼져 홀드가 취소되면 예약 발동하지 않는다.
- 반지름은 Step 16.1의 소비 후 길이 기반 반지름에 1.5를 곱한다. 회전 방향·10% 버림 소비·1초 주행·0.5초 원 경로 집결·전 구간 무적·10초 기본 재사용은 유지한다.
- 1초 주행의 시간 진행도는 사인 ease-in-out을 사용해 초반·후반을 느리게, 중간을 빠르게 만든다.
- Bomb 원 주행은 `WorldBlocker`와 월드 경계를 무시하며 종료 시 발동 원점으로 정확히 돌아온다.
- 폭발 피해는 발동 직전 몸통 수 × 몸통당 10 × `bomb_damage` 배율이다. 기존 +20%/단계·최대 3단계는 몸통당 피해에 적용한다.
- 웨이브 기본 구간·시간 성장·목표 활성 수·180 하드캡은 유지하고, `OSWaveScheduleData`의 전역 생성 밀도 배율 1.2를 스폰 예산에 추가 적용한다.

### 구현 현황

- 상태: 완료
- 최근 갱신: 2026-07-24
- 완료: `OSBombMath`에 반지름 1.5배·사인 완급·몸통 비례 피해 계산을 추가하고, `OSPlayerController`의 Bomb 주행을 장애물·경계 비충돌 실제 원 이동으로 전환했다. `OSInputRouter`와 세션은 B 시작/취소를 홀드 프리뷰로 전달하고 릴리스에서만 요청하며, `OSBombController`/`OSBombView`는 매 프레임 완성 원 프리뷰와 발동 직전 피해 스냅샷을 사용한다. `OSWaveScheduleData`와 런타임 복사본에 1.2 밀도 배율을 추가하고 `OSWaveDirector` 스폰 예산에 연결했다. Step 16.2 재적용·WebGL 빌드 메뉴와 집중 회귀를 추가했다.
- 남음: 없음. Step 전용 커밋·원격 푸시 뒤 다음 사용자 플레이 피드백을 기다린다.
- 검증: Unity 6000.5.1f1 컴파일 및 최종 Console Error/Exception 0. Bomb 계산·웨이브 집중 EditMode 16/16(job `ece992bf85764742a596005f35ee5383`), Bomb·입력 집중 PlayMode 13/13(job `e15bec0edde64b94afb5ac884edcd139`), 실제 씬 Bomb 집중 PlayMode 2/2(job `86dac843a3f646b3ae8ee89ff5ffc2fb`) 통과. 최종 전체 EditMode 74/74(job `85bf628527fa4468a952acff71fb2a33`)와 PlayMode 129/129(job `93b827e3d6b242ba8eb36464fc6a123e`)이 통과했다. 승인 `WebGL Development` Profile의 `Builds/Step16_2/WebGL/` 빌드는 errors 0·warnings 3·135,773,859 bytes로 성공했고 Windows 빌드는 실행하지 않았다. HTTP에서 `index.html`과 `WebGL.wasm` 200, WASM MIME `application/wasm`, WASM 120,964,940 bytes와 Canvas 960×540을 확인했다. MainMenu→Shield 역할 2회→Combat에서 `BODY 2`, `BOMB NEEDS 10 BODY [B]`와 B 입력의 몸통·세션 상태 무변경을 확인했다. 브라우저 Error는 0건이고 기존 URP FSR 비지원 Warning 1건만 기록됐다. 10칸 이상 B 홀드 실시간 완성 원·릴리스 발동, 사인 완급, 벽 통과 원점 복귀, 비례 피해는 집중/전체 PlayMode 자동 회귀로 확인했다.

### 체크리스트

- [x] 소비 후 길이 기반 반지름 1.5배와 사인 ease-in-out 원 진행도
- [x] B 홀드 시작/취소 전달, 릴리스 1회 발동, UI 전환 시 예약 발동 방지
- [x] 현재 머리·방향·몸통 배치에 맞춘 완성 원 프리뷰 실시간 갱신
- [x] `WorldBlocker`·월드 경계 비충돌 원 주행과 정확한 원점 복귀
- [x] 발동 직전 몸통 수×몸통당 10 피해와 강화 배율 스냅샷
- [x] SO 기반 일반 적 생성 밀도 1.2배와 기존 시간 곡선·180 하드캡 유지
- [x] Step 16.2 재적용 메뉴·데이터·씬 배선·집중 회귀
- [x] 전체 EditMode/PlayMode 회귀와 최종 Console Error/Exception 0
- [x] WebGL Development 빌드와 HTTP Canvas·브라우저 수동 확인
- [x] Step 전용 커밋과 원격 푸시

### 완료 기준

- B를 누르는 동안 원 전체가 보이고 이동·방향 변화에 맞춰 갱신되며, 놓기 전에는 몸통·재사용·세션 상태가 바뀌지 않는다.
- 릴리스하면 1초 안에서 초반·후반보다 중간 구간이 빠르게 진행되고, 장애물·경계가 원과 겹쳐도 중단 없이 한 바퀴 뒤 발동 원점으로 돌아온다.
- 같은 강화 단계에서 몸통 20칸 Bomb의 피해가 10칸의 정확히 2배이고, 소비된 몸통도 해당 발동 피해 계산에 포함된다.
- 일반 적 스폰 예산은 동일 전투 시간·기본 구간에서 Step 16.1보다 20% 증가하되 목표 활성 수·180 하드캡·안전 위치·티켓 보류 규칙은 유지된다.
- 전체 회귀와 WebGL 빌드·HTTP·Canvas·브라우저 Console 및 실제 플레이 수용 기준을 통과한다.
...
- Step 16 사용자 플레이 피드백 반복 개선 완료와 전체 런 확정
...
- Step 18 사운드·VFX가 사용할 파티클·오버드로우·동시 효과음 예산을 기록한다.
- 성능 미달이면 병목과 재검증 상태를 남기고 Step 18 장식 연출 확대와 배포 승인을 보류한다.

---

## 19. 사운드·VFX·연출 — Step 18

### 목표

Step 17 성능 예산 안에서 핵심 정보 전달을 강화하고 장식이 규칙을 가리지 않게 한다.

### 선행 조건

- Step 17 성능 기준선과 연출 예산 확정
...
- Step 17에서 정한 CPU·GPU·메모리 예산을 넘으면 장식 수·수명부터 줄이고 판정과 텔레그래프는 유지한다.

### 완료 기준

- 소리를 끈 상태와 색을 구분하기 어려운 상태에서도 핵심 위험을 읽을 수 있다.
- 장식 VFX 제거 시 게임 규칙 판독성이 떨어지지 않는다.
- Step 17 피크 장면과 WebGL 메모리를 다시 측정해 성능 예산 회귀가 없음을 확인한다.

---

## 20. 커스텀 머리 목업·ComfyUI — Risk Track
...
| 13일 | 흡인/보호막/페이즈/제어 | 보스 150초 테스트 | 보호막/흡인 연출 | 보스 패턴 동시 수 제한 |
...
- Step 17 초기 성능 측정이 미달이면 Step 18 장식 연출 확대를 중지한다.

## 22.4 4주차 — 플레이 피드백·최적화·연출·QA·빌드 마감
...
| 일차 | 프로그래머 | 기획·콘텐츠 | 아트·UI | 일일 확인 |
| ---: | --- | --- | --- | --- |
| 피드백 반복 | Step 16.x 게임성 개선 | 사용자 플레이 피드백 분석·수용 기준 합의 | 영향받는 UI·판독성 보정 | 사용자 확인 뒤 다음 반복 또는 게임성 기준선 확정 |
| 16일 | Step 17 WebGL 프로파일·병목 수정 | 확정 게임성 수치 정리 | 연출 성능 예산·리소스 정리 | 성능 기준선·예산 확정 |
| 17일 | Step 18 연출 풀·동시 수 연결 | 수치 2차 확인 | 사운드/VFX 최종 연결 | 피크 장면·G6 재측정 |
| 18일 | Step 19 동시 사건·WebGL 포커스·메모리 | 외부 테스트 1 | HUD/튜토리얼 회귀 검수 | G3~G5 전체 통과 |
...
- Bomb을 강화할 때 피해, 범위, 소비율을 동시에 올리지 않는다.
...
- [ ] 머리 피해와 몸통 절단 분리
- [ ] 절단 지점부터 꼬리 제거
- [ ] B Bomb의 10% 버림 소비·1.5배 반지름·몸통 비례 피해 경계값 정확
...
> 플레이어가 Windows와 WebGL에서 시작 몸통 역할을 고르고, 이동과 자동 공격으로 적을 처치해 몸통을 늘리며, 몸통 피격 시 일부를 잃고, Space 몸통 수축 대시로 포위를 벗어나거나 B Bomb으로 꼬리 10%를 소비해 공간을 만든 뒤, 역할 선택과 로그라이트 업그레이드로 다시 성장하여 10분 보스를 상대하고, 사망 또는 클리어 결과를 확인한 뒤 오류 없이 재시작할 수 있다.
