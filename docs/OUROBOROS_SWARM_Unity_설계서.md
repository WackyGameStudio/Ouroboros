...
> 문서 목적: 현재 핵심인 **몸통 축적 → 유지/절단 위험 → 대시 또는 Bomb 방출 → 재성장**을 뱀서류·지렁이류·로그라이트의 장르 문법으로 보강하여 Unity 2D에서 바로 구현 가능한 규칙과 구조로 고정한다.
>
...
| Space 능력 | **[확정] 0.5초 동안 진행 방향으로 4.5 유닛 몸통 수축 대시** | Step 15.7에서 기존 포위 폭발을 제거하고 교체했다. |
| 대시 자원/공격성 | **[확정] 몸통·역할을 소비하지 않고 적 피해·플레이어 무적을 만들지 않음** | 이동기와 몸통 연출에 책임을 제한한다. |
| 대시 재사용/복귀 | **[확정] 발동 시점부터 2초 재사용, 종료 지점에 뭉친 뒤 머리의 실제 이동 경로만큼 자연 전개** | 정지 중 강제 복귀를 없애고 이동과 체인 복귀를 하나의 규칙으로 읽게 한다. |
| B 능력 | **[확정] 몸통 10칸 이상에서 `floor(N×0.10)`을 소비하는 Bomb 원형 폭발** | 이동기와 별개의 공격적 몸통 방출 선택지를 만든다. |
| Bomb 재사용/피해 | **[확정] 발동 시점부터 기본 10초 재사용, 고정 범위 피해** | 몸통 길이는 원의 크기에만 반영하고 한 번의 폭발 피해는 적마다 1회만 적용한다. |
...
- Space 대시는 몸통 전체를 머리로 빠르게 끌어당기며 머리를 진행 방향으로 이동시킨다.
- 대시는 몸통 수·역할·지속 화력을 보존하고 `WorldBlocker`를 관통하지 않는다.
- B Bomb은 몸통 일부를 전술적으로 희생해 남은 몸통 길이의 원을 그리고 군집을 폭발시킨다.
- 플레이어는 다시 몸통을 모아 전력을 재건한다.
- 맵 탐색보다 군집 밀도, 안전 공간, 몸통 상태, 대시와 Bomb 타이밍을 읽는다.

### 0.3.1 Step 15.7 이전 포위 폭발 규칙의 폐기

**[확정]** 2026-07-19부터 Space에 연결됐던 `Explosion`, `Blast`, 꼬리 예약 원 기반 포위 폭발은 구현 계약에서 제거됐다. 이 문서의 과거 일정·회고 문맥에 남아 있는 해당 용어는 역사 기록일 뿐이며 Space 입력·수치·클래스·테스트는 `5.3 몸통 수축 대시`를 따른다. Step 16.1의 B Bomb은 머리가 실제 원을 주행하고 몸통이 그 경로를 따르는 별도 능력으로, 제거된 Space 포위 폭발 경로를 복구한 것이 아니다.
...
| --- | --- | --- |
| 자동 공격 | 머리와 공격형 몸통이 유효 표적을 자동 선정한다. | 플레이어 입력을 이동·포지셔닝·대시 판단에 집중시킨다. |
| 시간 기반 압박 | 60초 단위로 적 조합을 확장하고 생성 압력을 높인다. | 몸통을 오래 유지할수록 커지는 탐욕과 포위 위험을 만든다. |
| 대량 적 군집 | 개별 AI보다 밀도와 접근 방향을 읽게 한다. | 대시의 위치 재조정 가치를 강화한다. |
| 경험치 수집과 3택 | 레벨업 시 전투를 멈추고 중복 없는 업그레이드 3개 중 하나를 고른다. | 런마다 지속 화력·대시·생존 비중이 달라진다. |
...
- 화면의 적 수가 많아질수록 투사체 수만 늘리기보다 관통, 범위 공격, 제어, 대시가 의미를 갖게 한다.
...
- 20~30초마다 `몸통 생성`, `대시 재배치`, `정예/패턴 대응`, `레벨업` 중 최소 하나의 의미 있는 판단이 발생하도록 웨이브를 조정한다.
...
| 몸통 수축 | 대시 중 모든 몸통이 머리 중심으로 빨려들고, 종료 뒤 머리를 따라 이동한 거리만큼 다시 펴진다. | 긴 몸의 위험을 일시적으로 압축하는 이동 판단을 만든다. |
...
- 새로 얻은 세그먼트는 꼬리에 추가되며 대시로는 사라지지 않는다. 역할 선택 카드에 `꼬리에 추가됨`과 `대시 중 보존`을 명시한다.
- 절단으로 사라지는 세그먼트는 역할 문양과 연결선을 꼬리 방향으로 지워 상실 범위를 0.5초 안에 파악하게 한다.
...
| 빌드 방향 | 지속 화력, 몸통/역할, 대시, Bomb, 생존, 유틸 계열을 누적한다. | 몸통 역할 선택과 일반 강화가 결합된다. |
| 상황 적응 | 현재 몸통 조합과 생존 상태에 맞는 선택을 할 수 있다. | 고정 정답 대신 런마다 다른 복구 전략을 만든다. |
| 결과 회고 | 생존 시간, 최대 몸통, 절단 손실, 대시·Bomb 사용과 몸통 소비, 보스 결과를 보여준다. | 다음 런에서 판단을 개선할 근거를 준다. |
| 결정적 QA 시드 | 내부 테스트에서는 런 시드를 기록하고 재현한다. | 물리·선택·웨이브 결함을 반복 검증한다. |

### 추가 설계 원칙

- 무작위성은 **선택 후보**에만 강하게 사용한다. 이동, 피격, 대시, 역할 상실 결과는 결정적이어야 한다.
...
- 희귀도, 합성, 리롤, 밴시, 영구 해금은 제외한다. 17종의 작은 카탈로그로 선택의 질을 먼저 검증한다.
...
4. 포위가 완성되기 전에 진행 방향을 잡고 몸통 수축 대시로 안전 위치를 만든다.
5. 충분히 성장했으면 몸통 일부를 Bomb에 투자해 군집을 정리할지 판단한다.
6. 절단·Bomb 소비 후 남은 빌드로 재성장 동선을 만든다.
...
넓은 2D 필드에서 자동 공격과 자유 이동으로 군집 감염체를 처치하고 역할형 몸통을 늘린 뒤, 몸통 수축 대시로 포위를 벗어나거나 Bomb으로 군집을 정리하며 10분 보스를 격파한다.
...
| 시간 초과 | **[확정] 보스 등장 후 전투 시간 150초(2분 30초)가 경과** | 실패 결과로 종료; 선택 상태에서는 제한 시간이 정지 |
...
| 성장 | 1:30~3:00 | 역할 몸통 성장, 첫 대시 경험 | 돌진체 추가 | 유지/기동의 첫 고민 발생 |
| 정예 1 | 3:00 전후 | 정예 대응, 역할 조합 확인 | 가속 오라 | 집중 공격과 공간 확보 학습 |
| 분기 | 3:00~6:00 | 지속 화력·대시·제어 비중 형성 | 사격체·분열체 추가 | 서로 다른 빌드가 성립 |
| 정예 2 | 6:00 전후 | 손실 후 재구성 | 복합 군집 | 절단 후 회복성 검증 |
| 한계 | 6:00~9:00 | 긴 몸의 위험과 최대 밀도 대응 | 고밀도 혼합 웨이브 | 판독성과 성능 피크 검증 |
| 보스 예고 | 9:00~10:00 | 보스 전 몸통·HP 재정비 | 소환 압박 | 마지막 선택과 대시 재배치 판단 |
| 결전 | 10:00~최대 11:30 | 축적 화력과 대시를 함께 사용 | 군집핵 패턴 | 전체 빌드의 최종 검증 |

## 2.4 핵심 지표

- 첫 플레이 60초 이내 이동과 자동 공격 이해율
- 첫 대시까지 걸린 시간
- 첫 Bomb까지 걸린 시간과 몸통 수
- 대시 평균 사용 간격과 장애물 충돌 비율
- Bomb 평균 사용 간격·거부 사유·사용당 소비 몸통·고유 적 명중 수
- 대시 직후 2초 내 머리 피격·몸통 절단 비율
- 평균·최대 몸통 수와 절단으로 잃은 수
- 몸통 역할 선택 분포와 절단으로 사라진 역할 분포
- 3분·6분 정예 도달률
- 첫 플레이 생존 4~6분, 세 번째 플레이 보스 도달 목표
- 지속 화력 중심과 기동 대시 중심 빌드의 보스 처치 시간 차이 20% 이내
...
| --- | --- | --- | --- | --- |
| `Player/Move` | Value / Vector2 | WASD, 방향키 | Combat, BodyDash, Bomb | 크기 1로 제한하고 대각선 속도를 정규화한다. Bomb 중에는 새 입력으로 원 경로를 바꾸지 않는다. |
| `Player/BodyDash` | Button / Press | Space | Combat | 현재 이동 입력, 없으면 마지막 방향을 스냅샷으로 잡고 1회 발동한다. 대시·재사용 중 재입력은 거부한다. |
| `Player/Bomb` | Button / Press | B | Combat | 몸통·재사용·원 경로 안전성을 모두 만족할 때 1회 발동한다. 거부 입력은 몸통과 재사용 시간을 소비하지 않는다. |
...
- Map 전환 시 이동·대시·Bomb 입력 상태를 비우며 복귀 후 예약 실행하지 않는다.
...
- 머리의 시각 Sprite는 물리용 머리 Transform과 분리해 마지막 유효 이동 방향으로 회전한다. 현재 임시 `Head.png`의 눈이 향하는 원본 기준은 로컬 +Y(`90°`)이며 리소스 교체 시 `spriteForwardDegrees`만 새 원본의 정면 축에 맞춘다.
...
- 게임 플레이 카메라는 직교 크기 **6.5**의 고정 줌을 사용한다. 16:9 기준 약 23.1×13.0 월드 영역을 표시하며 HUD 크기는 변경하지 않는다.
- 긴 몸통 전체를 담기 위한 자동 줌아웃은 사용하지 않는다.
- 부드러운 추종 지연은 **[가설] 0.08~0.15초** 범위에서 조정한다.
- 위험 예고가 화면 가장자리에서 잘리지 않도록 카메라 여백 안쪽에 최소 표시 아이콘을 제공한다.
- 화면 흔들림은 대시 시작·보스 강공격에만 짧게 사용하고 옵션으로 강도를 낮출 수 있게 한다.
...
- 대기 중 생성 요청은 현재 몸통 수, 머리 화력, 대시 수축 몸통 수에 포함하지 않는다.
...
- 게이지가 6 이상이면 6을 차감하고 몸통 생성 요청 1건을 큐에 넣는다.
- 한 번에 여러 기준을 넘으면 요청을 여러 건 만든다.
- 대시 중에는 요청만 큐에 저장하고 선택 UI를 열지 않는다.
- 활성 세그먼트와 대기 요청 합이 64에 도달하면 게이지를 6 충족 상태로 보류하고 추가 조각을 누적하지 않는다.
- 절단으로 여유가 생기면 보류된 요청 1건을 즉시 생성한다.
...
- 생성·절단 직후 다음 머리 발사부터 새 길이를 사용한다. 대시는 길이 화력을 바꾸지 않는다.
...
- `SeveredBody`는 공용 픽업 도형이 아니라 원래 Shield/Attack/Laser/Control 몸통 스프라이트를 사용한다. 표시 스케일은 0.42로 활성 몸통 0.72보다 작게 유지하고 수집 Collider 크기는 바꾸지 않는다.
- `SeveredBody`를 머리로 수집하면 조각 게이지나 역할 선택을 거치지 않고 원래 역할 몸통 1칸을 꼬리에 즉시 복구한다. 한 픽업에서 여러 칸을 회수해도 모든 신규 세그먼트 View는 수집 순간의 기존 마지막 꼬리 위치(기존 몸통이 없으면 머리 위치)에 겹쳐 생성한다.
- 겹친 회수 세그먼트는 별도 시간 보간으로 펼치지 않는다. 머리가 실제로 이동해 새 경로를 만들면 세그먼트 간격 1칸에 해당하는 누적 거리마다 머리에 가까운 회수 세그먼트부터 정상 경로에 합류한다. 수집 순간 과거 경로 뒤쪽으로 즉시 확장하지 않는다.
- 기술 가드 때문에 일부만 복구되면 남은 수량은 픽업에 보존한다.
- 픽업 풀이 포화되면 같은 역할의 기존 `SeveredBody`에 수량을 합치고, 합칠 대상도 없으면 역할별 고정 대기량으로 보존해 픽업 반환 뒤 다시 생성한다.
- 절단 시 역할 관리자 등록 해제, 공통 화력 갱신, HUD 갱신을 하나의 원자 처리로 수행한다.

## 4.8 동일 공격의 머리·몸통 중복

- 한 공격 이벤트가 머리와 몸통 콜라이더에 동시에 닿으면 머리 피격 후보 1건만 남긴다.
- 서로 다른 공격 이벤트가 같은 틱에 머리와 몸통을 각각 맞히면 머리 피해를 먼저 처리한다.
- 머리 피해로 사망하면 그 틱의 몸통 절단, 대시 완료, 픽업, 선택 요청은 처리하지 않는다.
...
| 범위 | 세그먼트 중심 반경 1.5 |
| 충전 | 세그먼트당 1회 |
| 방어 대상 | 실제 접촉점이 범위 안인 머리 HP 피해 또는 몸통 절단을 만들 유효 적대 피격 1건 |
| 재충전 | 전투 시간 6초 |
| 중첩 | 피격점과 가장 가까운 충전 실드 1개만 소비; 동률이면 머리에 가까운 세그먼트 우선 |
| 제거 | 절단 시 범위, 충전, 재충전 타이머 즉시 제거; 대시에는 보존 |

- 이미 머리 무적이나 절단 방지로 무효인 피격에는 충전을 사용하지 않는다.
- 접촉 공격과 적 투사체의 피격점은 대상 세그먼트 중심이 아니라 충돌한 `Collider2D`에서 공격원에 가장 가까운 점으로 계산한다. 콜라이더를 얻지 못한 테스트·복구 경로에서만 대상 Transform 중심으로 대체한다.
...
| 판정 | 길이×폭 직사각형과 `EnemyHurtbox` Collider2D가 조금이라도 겹치는 모든 고유 적에게 1회 관통 피해 |
| 표시 폭 | `LineRenderer.widthMultiplier = 1`, 시작/끝 폭은 판정 스냅샷의 전체 폭과 동일 |
| 제거 | 예고 중 제거되면 해당 레이저 취소 |

- 예고 시작 시 시작 위치와 방향을 스냅샷으로 저장한다.
- 발사 시점에 소스 세그먼트 생존 여부를 다시 확인한다.
- 판정은 적 Transform 중심점이 아니라 예고와 동일한 시작점·방향·폭·길이의 사각형과 실제 `EnemyHurtbox` Collider2D 교차를 사용한다.
- 레이저 예고·발사 `LineRenderer`의 화면상 전체 굵기는 스냅샷의 `Width`와 같아야 하며 별도 `widthMultiplier`로 확대·축소하지 않는다.
...
- 세션 상태가 `Combat`이다.
- 몸통 수와 무관하게 발동할 수 있다.
- 대시 중이 아니며 재사용 대기 시간이 0이다.
- 선택, 사망, 클리어, 결과 상태가 아니다.
...
| 지속 시간 | **[확정] 0.5초** |
| 기본 거리 | **[확정] 4.5 유닛** |
| 재사용 대기 | **[확정] 발동 시점부터 2초** |
| 몸통 자연 전개 | **[확정] 별도 타이머 없음; 대시 종료 후 머리의 실제 이동 거리만 사용** |
| 대시 픽업 흡입 속도 | **[가설] 24 유닛/초** |

- 발동 순간의 `Move` 입력이 0이 아니면 정규화한 방향을 사용한다.
- 이동 입력이 없으면 `OSPlayerController.LastDirection`, 그것도 없으면 오른쪽을 사용한다.
- 방향은 발동 순간에 고정하며 대시 중 입력으로 꺾지 않는다.

### 머리 이동

1. `OSGameSessionController`가 `BodyDash`로 전이하고 시뮬레이션과 Player Map을 유지한다.
2. `OSPlayerController`가 0.5초 ease-out 곡선으로 총 4.5 유닛을 이동한다.
3. 모든 이동은 평상시와 같은 Kinematic `Collider2D.Cast`와 접선 미끄러짐을 사용한다.
4. `WorldBlocker` 또는 월드 경계에 막히면 남은 정면 이동을 관통하지 않는다.
5. 0.5초가 끝나면 실제 이동 거리를 결과에 기록하고 `Combat`으로 복귀한다.

### 몸통 수축과 자연 전개

- 발동 시 활성 세그먼트의 시작 위치를 고정 배열에 복사한다.
- 0.5초 동안 모든 활성 세그먼트 View를 매 틱의 현재 머리 위치로 ease-out 보간해 빨려들어가는 느낌을 만든다.
- 대시 종료 시점에는 활성 몸통 View가 머리 중심에 모여야 한다.
- 대시 종료 시 경로 버퍼를 도착한 머리 위치로 다시 고정하고, 머리가 정지해 있는 동안에는 모든 몸통이 그 위치에 뭉친 상태를 유지한다.
- 이후 머리가 평상 이동으로 새 경로를 만들면 각 세그먼트가 기존 간격에 도달한 순서대로 머리 뒤를 따라 자연스럽게 다시 펴진다. 별도 0.25초 재전개 보간은 사용하지 않는다.
- 경로 재고정은 안정 ID, 역할, 몸통 수, 발사 상태를 변경하지 않는다.

### 전투·선택 규칙

- 대시는 적에게 피해·제어·넉백을 주지 않는다.
- 대시는 몸통을 제거하거나 절단 몸통 픽업을 만들지 않는다.
- 대시 시작·이동·종료는 새 `OSPickup`이나 수집 가능 표식을 생성하지 않는다.
- 머리가 실제로 이동한 각 Cast·슬라이드 구간을 현재 유효 자석 반경으로 쓸고, 그 범위에 중심이 들어온 기존 경험치·몸통 조각·회복·절단 몸통 픽업은 대시 흡입 상태로 고정한다.
- 대시 흡입 픽업은 즉시 효과를 적용하거나 사라지지 않고 24 유닛/초로 머리를 빠르게 추적한다. 대시가 끝나 자석 반경 밖이 되어도 추적을 유지하며 머리 `PickupCollector`에 실제로 닿을 때 기존 종류별 수집·부분 회수 규칙을 적용한다.
- 대시는 머리 피격 무적이나 몸통 절단 방지를 제공하지 않는다.
- 대시 중 발생한 Body/LevelUp 요청은 큐에 보존하고 대시가 끝난 뒤 Body 우선으로 연다.
- 대시 중 사망·클리어가 확정되면 머리 이동과 몸통 수축을 즉시 취소하고 후속 선택을 열지 않는다.

### 금지 사항

- 대시를 `Transform.Translate`나 순간이동으로 구현해 장애물을 관통하지 않는다.
- 대시를 위해 몸통 Transform을 경로 데이터 자체에 덮어쓰지 않는다.
- 조건 불충족 입력으로 쿨다운을 소비하지 않는다.
- 제거된 Space 포위 폭발의 예약 원과 `OSExplosion*` 경로를 대시에 다시 연결하지 않는다. B Bomb은 `5.4`의 별도 입력·상태·원 주행 계약만 따른다.
...
1. 소비 전 활성 몸통의 위치를 머리 진행 방향 기준 외적으로 좌·우 분류한다.
2. 왼쪽 수가 많으면 오른쪽으로, 오른쪽 수가 많으면 왼쪽으로 회전해 몸통이 적은 쪽으로 원을 시작한다.
3. 좌·우 개수가 같으면 모든 몸통의 부호 있는 횡방향 거리 합을 사용해 더 크게 치우친 쪽의 반대로 회전한다.
4. 횡방향 합도 정확히 같으면 오른쪽 회전을 사용한다.
...
- 발동 성공부터 1초 원 그리기와 0.5초 집결이 끝날 때까지 머리 피해와 몸통 절단을 모두 무시한다.
- 능력 무적으로 막힌 공격에는 Shield 충전을 소비하지 않는다. 기존 피격 무적 타이머는 독립적으로 유지한다.
- Bomb 중 발생한 Body/LevelUp 요청은 큐에 보존하고 집결 완료 뒤 Body 우선으로 연다.
- 사망·클리어·재시작·오브젝트 비활성화로 중단되면 원 이동·VFX·능력 무적을 정리하고 후속 폭발이나 선택을 실행하지 않는다.
- 재사용 대기는 발동 성공 시점부터 진행하며 선택으로 `timeScale`이 0인 동안에는 줄지 않는다.
...
- 피격 무적 0.6초 또는 Bomb 능력 무적이 유효하면 머리 피해를 막는다. 대시는 별도 무적을 제공하지 않는다.
- 무적 중 피격은 HP, 무적 시간, 실드 충전을 변경하지 않는다.
- 회복은 최대 HP를 넘지 않는다.
- HP가 0 이하이면 즉시 `Dead`로 전이한다.
- 사망 틱의 후속 절단, 대시 완료, 픽업, 레벨업, 역할 선택은 취소한다.
...
| 절단 몸통 | 원래 역할 몸통 즉시 재부착 | 현재 마지막 꼬리에 겹쳐 생성 후 이동 거리로 순차 전개; 조각 게이지·역할 선택 없음; 기술 가드 초과분은 픽업에 잔류 |

- 모든 픽업은 머리의 수집 콜라이더만 수집한다.
- 픽업은 자석 반경 밖에서는 정지 또는 짧은 드롭 이동만 하고, 반경 안에서 머리로 가속한다.
- 대시 이동 구간의 유효 자석 반경에 걸린 픽업은 대시 흡입으로 고정되어 반경을 벗어나도 24 유닛/초로 머리까지 계속 추적한다. 이 상태도 수집 자체는 머리 콜라이더 접촉으로만 확정한다.
...
3. 같은 종류 병합 대상이 없고 요청 좌표가 다른 종류를 포함한 활성 픽업과 겹치면, **[가설] 최소 0.55 간격**을 만족하는 결정적 원형 후보를 비할당 탐색한다.
4. 분산된 좌표로 풀에서 새 픽업을 대여한다.
5. 풀이 포화되면 가장 가까운 같은 종류 픽업에 거리를 무시하고 값을 합친다.
6. 값은 보존하고 GameObject 수만 줄인다. 분산은 종류와 수량을 변경하지 않으며 같은 종류 병합보다 먼저 실행하지 않는다.

Step 12 기준 경험치·몸통 조각·회복과 Step 15.5의 절단 몸통은 세션 전 256개를 만든 공용 `body_fragment_pickup` 풀을 사용한다. 풀 키는 구현 호환을 위해 유지하되 각 인스턴스는 `OSPickupType`으로 효과·병합 대상을 구분한다. 몸통 조각은 청록색 `Body_Control` 마름모, 경험치는 금색 `Projectile` 원형, 회복은 초록색 `Pickup` 십자 형태로 표시해 색과 실루엣을 모두 다르게 한다. `SeveredBody`는 원래 Shield/Attack/Laser/Control 몸통 스프라이트를 작은 크기로 표시하고 같은 역할끼리만 용량 병합하며, 풀 반환·재대여 시 현재 타입에 맞는 스프라이트와 색상으로 다시 설정한다.
...
| `body_fragment_efficiency` | 몸통 | 증식 효율 | 조각 요구량 -17% | 2 | 기본 6, 단계별 5→4, 최소 요구량 4 |
| `body_damage_rate` | 몸통 | 연결 증폭 | 칸당 머리 피해 +1%p | 2 | 4% → 최대 6% |
| `role_overclock` | 몸통 | 역할 과충전 | 역할 주기·실드 재충전 -8% | 3 | 최소 주기/재충전 가드 적용 |
| `dash_distance` | 대시 | 신경 도약 | 대시 거리 +15% | 3 | 기본 4.5에 배율 적용 |
| `dash_cooldown` | 대시 | 재점화 | 대시 재사용 시간 -12% | 3 | 배율 하한 50% |
| `dash_recharge` | 대시 | 추진 재충전 | 대시 재사용 시간 -0.2초 | 3 | 기존 배율 적용 뒤 합산, 최저 0.5초 |
...
- 레벨업 3택 카드는 `계열 → 강화 이름 → 쉬운 효과 문장 → CURRENT → AFTER 실제 수치 → 현재/다음/최대 단계` 순서로 표시한다.
- 쉬운 효과 문장은 영향을 받는 게임 요소와 변화 방향·단계당 변화량을 함께 설명한다. 내부 ID나 원시 배율만 노출하지 않는다.
- 현재→적용 후 수치는 현재 밸런스 SO와 하한·상한을 반영한 값이며 피해·주기·개수·거리·HP 등 의미와 단위를 생략하지 않는다.
- 최대 HP의 증가분 회복, 대시 재사용 시간 최저 0.5초, 이동 속도 상한 7.5처럼 선택 해석에 필요한 제한은 효과 문장에 함께 표시한다.
- 선택 중 전투 정지와 카드 클릭 1회 확정 규칙은 유지하며 일반 Submit으로 선택을 자동 완료하지 않는다.
...
| 기동 수축 | 대시 거리·배율 재사용·고정 재사용 감소, 조각 효율 | 제어·실드 혼합 | 포위 이탈과 긴 몸 재배치 | 대시 자체 공격력 없음 |
| Bomb 방출 | Bomb 피해·재사용, 조각 효율 | 공격·레이저 중심 | 성장한 몸통을 광역 피해로 전환 | 반복 사용 시 역할·지속 화력 손실 |
| 안전 재건 | 이동·HP·회복·자석·역할 과충전 | 실드·제어 | 절단 후 회복 안정 | 순간 처치력 부족 |
...
| `enemy_splitter` | 분열체 | 밀도 증가 | 36 | 1.5 | 접촉 7 | 사망 균열 표시 | 레이저·관통, 대시 이탈 |
...
SpawnRateMultiplier(minute <= 3) = 1.15 ^ minute
...
| ID | `boss_swarm_core` |
| HP | **[확정] 6,000** |
| 제한 시간 | **[확정] 전투 시간 150초(2분 30초)**; 선택 중 정지 |
| 보호막 HP | **[확정] 600** |
| 이동 | 느린 추적 + 패턴 위치 보정 |
...
| 패턴 | 예고 | 효과 | 플레이 대응 |
| --- | --- | --- | --- |
| 흡인 펄스 | 중심 링 0.8초 | 머리를 충돌 안전 이동으로 중심 쪽 2.2만큼 당김 | 외곽 이동, 대시로 재배치 |
| 부채꼴 탄 | 방향 부채 0.6초 | 1·2페이즈 5발, 3페이즈 7발의 피해 14 탄 | 부채 사이 이동 |
| 군집 소환 | 외곽 링 0.8초 | 1·2페이즈 추적·분열체 6마리, 3페이즈 8마리 | 관통·레이저·대시 이탈 |
...
보스는 대시 연타나 지속 화력 한쪽만 정답이 되지 않게 한다. 대시는 위험 위치를 벗어나는 수단이고, 유지한 몸통은 소환 군집과 보호막·본체를 제거하는 실제 화력을 담당한다.

**[확정]** `OSBossEncounterController`는 플레이어 피해 확정 뒤에 보스 사망을 판정한다. 같은 틱에 플레이어와 보스가 모두 사망하면 플레이어 사망이 우선하며 별도 동시 승리는 없다. 보스 처치, 플레이어 사망, 150초 시간 초과는 각각 구분되는 결과 종류로 런 요약에 고정한다.
...
| Bomb | 몸통 영역 | B 키, 필요 몸통, 준비/재사용/원 그리기/집결, 현재 고정 피해 | `BombStarted/BombExploded/BombCompleted` |
...
- 적 150마리 상황에서도 머리 코어, 위험 공격, 경험치, 대시·Bomb 상태가 구분되어야 한다.
...
- `꼬리에 추가` 라벨
- 몸통은 대시로 소비되지 않는다는 보조 설명은 첫 1회만 노출
...
3. Bomb 원 경로·폭발 영역과 대시 몸통 수축 상태
4. 몸통 절단 경계와 실드 충전
5. 일반 공격 이펙트
6. 장식 파티클
...
| 3 | 첫 몸통 조각 획득 | 조각 6개로 역할 몸통 생성 | 첫 역할 선택 완료 |
| 4 | 첫 역할 몸통 확정 | Space 몸통 수축 대시 안내 | 첫 대시 또는 20초 경과 |
...
| 몸통 | 최대 몸통, 최종 몸통, 획득 수, 절단 손실, 대시에서 수축한 누적 몸통 수, Bomb 소비 몸통 수 |
| 전투 | 총 처치, 정예 처치, 대시 사용 수, Bomb 사용·고유 적 명중 수, 받은 머리 피해 |
...
**[확정]** 판독성 Sorting Order 기준은 머리 코어 220, 적·보스 즉시 위험 200, 레이저 198, Bomb 원 190, Bomb 폭발 채움 189, 절단 경계 185, 제어 잔여 원호 182, 실드 175다. 대시는 별도 피해 범위 View 없이 몸통 수축과 HUD 상태로 표시한다. Bomb 원은 1초 진행도를 둘레로 그리고 폭발 순간 내부 채움을 짧게 표시하되 실제 Collider 판정과 분리한다. 제어 잔여량은 적 둘레의 감소 원호로도 표시하고, 절단 피드백은 충돌 위치의 경계와 꼬리 방향 및 역할별 상실 수를 한 메시지로 축약한다. 표시 오브젝트나 피드백이 부족하거나 비활성화되어도 전투 판정은 되돌리지 않는다.
...
| Combat | 진행 | On | Off | 대시/Bomb/선택/사망/클리어 |
| BodyDash | 진행 | On | Off | 0.5초 완료 또는 사망/클리어 |
| Bomb | 진행 | On | Off | 1초 원 그리기 + 0.5초 집결 완료 또는 사망/클리어 |
...
- 선택과 결과에서 0, Combat/BodyDash/Bomb에서 1을 사용한다.
...
| `OSInputRouter` | MonoBehaviour | Move·BodyDash·Bomb와 UI Input Action 구독, Player/UI Map 전환, 입력 버퍼 제거 |
...
| `OSBossEncounterController` | MonoBehaviour | 10:00 보스 1회 생성, 선택 중 제한 시간 정지, 플레이어 피해 뒤 보스 사망/150초 시간 초과 판정을 세션 결과로 전달 |
| `OSRunSummaryController` | MonoBehaviour | 런의 몸통·처치·피해·역할·업그레이드·대시·Bomb·시드/데이터 통계를 누적하고 종료 시 불변 요약을 생성 |
...
| `OSBodyChain` | MonoBehaviour | 경로 버퍼, 세그먼트 순서, 생성·절단·Bomb 꼬리 소비, 회수 묶음의 마지막 꼬리 경로 고정·이동 기반 순차 전개, 대시 수축·Bomb 원 경로 집결·도착점 경로 재고정 |
...
flowchart LR
    ...
    Session --> Dash[OSBodyDashController]
    Dash --> Player
    Dash --> Chain
    Session --> Bomb[OSBombController]
    Bomb --> Player
    Bomb --> Chain
    Bomb --> Registry
...
`OSUpgradeRunState`는 17종 단계에서 불변 `OSUpgradeModifiers` 스냅샷을 다시 계산하고 `OSLevelUpController`가 머리 무기, 성장, 역할 4종, 대시, Bomb, HP, 이동, 픽업 수집 계통에 배포한다. 적용 중 `OSUpgradeCatalog`나 다른 ScriptableObject의 직렬화 값은 수정하지 않는다.
...
| 6 | 대시 이동·몸통 수축 또는 Bomb 원 주행·집결 진행; 사망/클리어가 확정되면 즉시 취소 |
| 7 | 적 사망과 드롭 생성 확정 |
| 8 | 픽업 적용, 몸통/레벨업 요청 생성 |
| 9 | 보스 사망·세션 클리어 판정 |
| 10 | 대시 또는 Bomb이 끝났고 대기 요청이 있으면 Body 전부 → LevelUp 순서로 선택 상태 진입 |
...
- 시작 위치는 기술 가드와 같은 길이 64의 고정 `Vector2[]`에 복사한다.
- 대시·자연 전개 중 컬렉션 생성, LINQ, Collider 조회를 수행하지 않는다.
- 머리 이동은 기존 비할당 Cast 배열을 재사용한다.
- 대시 픽업 흡입은 `OSPickupSpawner`의 고정 활성 배열을 순회하고 점-선분 거리 제곱만 계산하며 물리 Overlap이나 임시 컬렉션을 만들지 않는다.
- 몸통 위치는 기존 경로 평가 결과와 시작 위치·머리 위치 사이 보간만 사용한다.
- 수축 연출 때문에 역할 Registry나 안정 ID 배열을 재구성하지 않는다.
...
- 적 180 / 투사체 120 / 몸통 40
- 적 180 / 투사체 120 / 몸통 64
- 몸통 64에서 대시 수축·도착점 뭉침·이동 기반 자연 전개
- 레이저 다중 발사 + 픽업 256
- 정예 오라 + 보스 소환 동시
...
| 조각 | 5/6/11/12, 64 가드, 여유 발생 후 보류 재개 |
...
| 동시 사건 | 같은 공격 머리+몸통, 다른 공격 동시, 절단+대시 완료 |
| 대시 픽업 | 실제 이동 구간의 자석 반경 안 전량 흡입 고정, 범위 밖 불변, 즉시 소멸 없이 고속 추적 |
| 사망 | 사망+픽업, 사망+대시, 선택 중 사망 요청 취소 |
...
- 첫 역할 조합과 첫 대시 시각
- 대시 방향·재사용 판단이 발생한 구체적 순간
- 절단 원인이 읽혔는지
- 대시 전에 진행 방향과 도착 지점을 예측했는지
- 대시 뒤 정지 중 뭉침과 이동에 따른 몸통 전개가 자연스럽게 읽혔는지
- 화면이 읽히지 않은 시각과 원인
- 선택지가 실제 빌드 방향을 바꿨는지
- 보스에서 대시와 지속 화력을 함께 활용했는지

### 코어 재미 합격 기준

- 테스터 70% 이상이 대시 방향과 재사용 타이밍을 실제로 고민했다고 응답한다.
- 대시 사용 전 진행 방향과 대략적인 도착 위치를 예측할 수 있다.
- 절단과 머리 피해 원인을 구분한다.
- 수축 뒤 몸통이 정지 중 도착점에 남고, 머리 이동을 따라 끊김이나 몸통 소실 없이 다시 펴진다.
...
| 몸통 생성 | 6조각마다 4택 1회, 다중 요청 직렬, 20칸 이후 성장 |
| 절단 몸통 회수 | 수집 순간 기존 마지막 꼬리에 전량 겹침, 머리 이동 거리 1칸마다 앞쪽부터 순차 전개, 역할·수량 보존 |
...
| 대시 거리 | 4.5 유닛/0.5초 | 회피로는 충분하면서 조작 위치를 잃을 만큼 긴가? |
| 대시 재사용 | 발동 시점부터 2초 | 연타가 지배적이거나 반대로 생존기로 너무 느린가? |
| 몸통 자연 전개 | 별도 타이머 없이 머리 이동 경로 사용 | 정지 중 뭉침과 이동에 따른 순차 복귀가 자연스럽게 읽히는가? |
| 기술 가드 | 64 | WebGL 안정성과 무제한 성장 체감 사이에 적절한가? |
| 보스 시간 | 150초(2분 30초) | 10분 빌드 검증에 충분한가? |
