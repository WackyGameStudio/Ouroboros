...
SpawnRateMultiplier(minute) = 1.08 ^ minute
```

**[확정]** Step 13 런타임은 `elapsedSeconds / 60`을 지수로 사용해 두 배율을 연속 적용한다. Step 15.9 플레이 체감 조정으로 생성 배율은 분당 `1.15`에서 `1.08`로 낮췄고, 일반 생성 구간의 `spawnRate`도 기존 값에서 20% 낮춘 `0.40, 0.60, 0.80, 0.96, 1.12, 1.28, 1.44, 1.60, 1.60, 1.60`을 사용한다. 3분·6분의 1초 정예 이벤트 구간은 생성률 0을 유지한다. `OSWaveScheduleData`와 `OSEncounterBalanceData` 원본은 세션 중 수정하지 않고 세션별 런타임 복사본과 스폰 시점 HP에만 반영한다.

**[확정]** 이 조정의 유효 생성률은 시작 시 초당 0.40, 5분 부근 초당 약 1.88, 10분 부근 초당 약 3.45다. 목표 활성 수와 180 하드캡은 안전 상한으로 유지하되, 부족한 적을 즉시 목표 수까지 채우는 보충 규칙으로 사용하지 않는다.
...
| `OSBodyChain` | MonoBehaviour | 경로 버퍼, 세그먼트 순서, 생성·절단, 대시 수축·도착점 경로 재고정·이동 기반 자연 전개 |
