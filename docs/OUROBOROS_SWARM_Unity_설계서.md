...
Step 12 기준 경험치·몸통 조각·회복과 Step 15.5의 절단 몸통은 세션 전 256개를 만든 공용 `body_fragment_pickup` 풀을 사용한다. 풀 키는 구현 호환을 위해 유지하되 각 인스턴스는 `OSPickupType`으로 효과·병합 대상을 구분하고 경험치/조각/회복 색상을 다르게 표시한다. `SeveredBody`는 원래 Shield/Attack/Laser/Control 몸통 스프라이트를 작은 크기로 표시하고 같은 역할끼리만 용량 병합하며, 풀 반환 시 공용 픽업 스프라이트로 복구한다.
...
**[확정]** `OSBossEncounterController`는 플레이어 피해 확정 뒤에 보스 사망을 판정한다. 같은 틱에 플레이어와 보스가 모두 사망하면 플레이어 사망이 우선하며 별도 동시 승리는 없다. 보스 처치, 플레이어 사망, 90초 시간 초과는 각각 구분되는 결과 종류로 런 요약에 고정한다.
...
| `OSBossEncounterController` | MonoBehaviour | 10:00 보스 1회 생성, 선택 중 제한 시간 정지, 플레이어 피해 뒤 보스 사망/90초 시간 초과 판정을 세션 결과로 전달 |
