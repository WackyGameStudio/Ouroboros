...
| 시간 초과 | **[가설] 보스 등장 후 90초가 경과** | 실패 결과로 종료; 10분 생존과 보스 DPS를 함께 검증 |
...
| 소환 상한 | 활성 일반 적 상한 안에서만 생성 |

### 패턴

| 패턴 | 예고 | 효과 | 플레이 대응 |
| --- | --- | --- | --- |
| 흡인 펄스 | 중심 링 0.8초 | 플레이어 머리를 중심 쪽으로 당김 | 외곽 이동, 폭발로 소환 정리 |
| 부채꼴 탄 | 방향 부채 0.6초 | 5~7발 느린 탄 | 부채 사이 이동 |
| 군집 소환 | 외곽 포털 0.8초 | 추적·분열체 생성 | 관통·레이저·폭발 |
| 보호막 | 코어 외곽막 0.5초 | **[가설] 600 HP의 피해 흡수** | 몸통 유지로 지속 파괴 후 본체 공격 |

### 페이즈

- 100~70%: 부채꼴 탄과 소환으로 기본 대응 확인
- 70~35%: 흡인 펄스를 추가해 폭발 타이밍을 압박
- 35~0%: 보호막과 소환 간격을 단축하되 화면 위험 신호 수는 제한
