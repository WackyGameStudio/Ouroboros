...
| 대시 재사용/회복 | **[확정] 발동 시점부터 2초 재사용, 종료 후 0.25초 몸통 재전개** | 연속 입력과 시각 복귀를 결정적으로 만든다. |
...
| 몸통 수축 | 대시 중 모든 몸통이 머리 중심으로 빨려들었다가 경로 위치로 재전개된다. | 긴 몸의 위험을 일시적으로 압축하는 이동 판단을 만든다. |
...
- 이후 0.25초 동안 현재 경로 샘플이 계산한 정상 위치로 재전개한다.
- 수축·재전개는 View 위치만 바꾸며 안정 ID, 역할, 몸통 수, 경로 버퍼, 발사 상태를 변경하지 않는다.
...
| `dash_recovery` | 대시 | 조직 재정렬 | 몸통 재전개 시간 -0.05초 | 3 | 최저 0.1초 |
...
| 기동 수축 | 대시 거리·재사용·재전개, 조각 효율 | 제어·실드 혼합 | 포위 이탈과 긴 몸 재배치 | 대시 자체 공격력 없음 |
...
| `OSBodyChain` | MonoBehaviour | 경로 버퍼, 세그먼트 순서, 생성·절단, 대시 수축·재전개 View |
...
- 대시·재전개 중 컬렉션 생성, LINQ, Collider 조회를 수행하지 않는다.
...
| 몸통 수축 | 시작 위치→머리 수축, 0.25초 경로 재전개, 몸통 수 불변 |
...
- 절단과 머리 피해 원인을 구분한다.
- 수축 뒤 몸통 재전개가 끊김이나 몸통 소실로 느껴지지 않는다.
...
| 몸통 재전개 | 0.25초 | 수축의 속도감과 원래 체인 복귀가 자연스럽게 읽히는가? |
