...
- 대시 시작·이동·종료는 어떤 `OSPickup`이나 수집 가능 표식도 생성하지 않으며, 이미 맵에 있던 픽업도 제거하거나 이동시키지 않는다.
...
| 몸통 수축 | 시작 위치→머리 수축, 정지 중 뭉침 유지, 이동 거리 기반 순차 전개, 몸통 수·픽업 수 불변 |
...
| 몸통 수축 대시 | Space 1회, 0.5초·4.5 유닛, 장애물 관통 없음, 몸통/역할/HP 불변, 2초 재사용 |
