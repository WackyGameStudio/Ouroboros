...
- Step 13 시간 웨이브를 다시 연결하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 13 Timed Waves`를 사용한다. `OSWaveDirector`만 세션별 `OSWaveScheduleRuntime` 복사본, 런 시드 13013, 전투 시간, 누적 스폰 티켓과 3·6분 정예/9분 경고를 소유하며 선택 중에는 진행하지 않는다. 일반 적은 카메라 밖·머리 거리 7 이상·월드 경계 안·`WorldBlocker` 밖 후보를 최대 8회 검사하고, 실패 또는 활성 일반 적 180 상한에서는 티켓을 다음 틱으로 넘긴다. HP와 생성률은 각각 `1.12^(elapsed/60)`, Step 15.9에서 완화한 `1.08^(elapsed/60)`을 SO 원본 변경 없이 적용하며 10:00 보스 이벤트는 Step 14의 `OSBossEncounterController`로 전달한다.
...
- Step 15.9 적 웨이브 생성 속도를 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.9 Slower Wave Pacing`을 사용한다. 일반 생성 구간의 `spawnRate`는 `0.40/0.60/0.80/0.96/1.12/1.28/1.44/1.60/1.60/1.60`이고 분당 연속 배율은 `1.08^(elapsed/60)`이다. 적 가중치, 목표 활성 수, 3·6분 정예, 9분 경고, 10분 보스, 180 하드캡과 안전 위치·티켓 보류 규칙은 바꾸지 않는다. 회귀는 `OSWaveScheduleEditModeTests`, `OSStep13ScenePlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_9/WebGL/`에 둔다.
- Step 14 이후 구현 순서는 Step 15 HUD·튜토리얼·전투 판독성 → Step 15.1 전장 충돌 → Step 15.2 실드·성장 밸런스 → Step 15.3 픽업 분산 → Step 15.4 레이저 Collider 범위 → Step 15.5 몸통 화력·절단 회수 → Step 15.6 절단 몸통 소형 비주얼 → Step 15.7 몸통 수축 대시 → Step 15.8 자연 전개 → Step 15.9 적 웨이브 생성 속도 완화 → Step 16 성능 최적화·WebGL → Step 17 사운드·VFX·연출 → Step 18 자동 테스트·빌드 게이트로 고정한다. Step 16에서 코어 전투의 성능 기준선과 연출 예산을 먼저 확정하고, Step 17은 그 예산 안에서 구현한 뒤 같은 피크 장면과 WebGL 메모리를 재측정한다.
