...
- Step 10 Space 능력 기반을 다시 연결하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 10 Body Convergence Dash`를 사용한다. 현재 규칙은 Step 15.7에서 교체된 `OSBodyDashController` 계약을 따르며, 과거 포위 폭발의 적 피해·몸통 소비·무적·예약 규칙과 `OSExplosion*` 타입을 복구하지 않는다. G0의 `OSEnemyDebugSpawner` 12마리 보충은 Step 13 적용 뒤 비활성 상태로 보존하고, 추적체 몸통 조각 드롭 가설 25%는 플레이테스트 결론 없이 변경하지 않는다.
...
- `OSLevelUpController`가 15종 업그레이드 단계와 불변 Modifier 스냅샷을 소유해 머리 무기, 몸통 성장·길이 화력, 역할 4종, 대시, HP·이동·회복, 자석·경험치, 정예 우선 표적에 배포한다. 발사 주기 0.15초, 기본 조각 요구량 6·성장 효율 단계 5/4, 대시 거리·재사용·재전개 하한, 이동 속도 7.5/s 하한·상한을 유지하고 SO 원본은 수정하지 않는다. 경험치·몸통 조각·회복·절단 몸통은 기존 `body_fragment_pickup` 256개 풀을 `OSPickupType`별 효과·병합·색상으로 공유하며 `SeveredBody`는 원래 역할과 수량을 추가로 보존한다. 레벨업 회귀는 `OSLevelUpCoreEditModeTests`, `OSLevelUpPlayModeTests`, `OSStep12ScenePlayModeTests`로 확인한다.
...
- Step 15.7 몸통 수축 대시를 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.7 Body Convergence Dash`를 사용한다. `BodyDash` 입력은 Space를 유지하고 전투 중 0.5초 동안 머리를 스냅샷 진행 방향으로 기본 4.5 이동시키며 `OSPlayerController`의 기존 비할당 Kinematic Cast와 월드 경계 제한을 그대로 적용한다. 같은 0.5초 동안 활성 몸통 View는 현재 머리 위치로 ease-out 수축하고 0.25초 동안 경로 위치로 재전개한다. 사용은 몸통 0칸에서도 가능하며 적 피해, 피격 무적, 몸통 제거·역할 변경을 만들지 않는다. 기본 재사용 대기시간은 발동 시점부터 2초다. 회귀는 `OSBodyDashMathEditModeTests`, `OSBodyDashControllerPlayModeTests`, `OSStep10ScenePlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_7/WebGL/`에 둔다.
- Step 14 이후 구현 순서는 Step 15 HUD·튜토리얼·전투 판독성 → Step 15.1 전장 충돌 → Step 15.2 실드·성장 밸런스 → Step 15.3 픽업 분산 → Step 15.4 레이저 Collider 범위 → Step 15.5 몸통 화력·절단 회수 → Step 15.6 절단 몸통 소형 비주얼 → Step 15.7 몸통 수축 대시 → Step 16 성능 최적화·WebGL → Step 17 사운드·VFX·연출 → Step 18 자동 테스트·빌드 게이트로 고정한다. Step 16에서 코어 전투의 성능 기준선과 연출 예산을 먼저 확정하고, Step 17은 그 예산 안에서 구현한 뒤 같은 피크 장면과 WebGL 메모리를 재측정한다.
