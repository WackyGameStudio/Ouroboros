...
- Step 14 이후 구현 순서는 Step 15 HUD·튜토리얼·전투 판독성 → Step 15.1 전장 충돌 → Step 15.2 실드·성장 밸런스 → Step 15.3 픽업 분산 → Step 15.4 레이저 Collider 범위 → Step 15.5 몸통 화력·절단 회수 → Step 15.6 절단 몸통 소형 비주얼 → Step 15.7 몸통 수축 대시 → Step 15.8 자연 전개 → Step 15.9 적 웨이브 생성 속도 완화 → Step 15.10 후반 웨이브 생성 속도 완화 → Step 15.11 게임 카메라 시야 확대 → Step 16 성능 최적화·WebGL → Step 17 사운드·VFX·연출 → Step 18 자동 테스트·빌드 게이트로 고정한다. Step 16에서 코어 전투의 성능 기준선과 연출 예산을 먼저 확정하고, Step 17은 그 예산 안에서 구현한 뒤 같은 피크 장면과 WebGL 메모리를 재측정한다.
