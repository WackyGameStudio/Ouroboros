...
- 웨이브 디렉터가 들어오기 전 G0 세로 슬라이스에서는 `OSEnemyDebugSpawner`가 200개 사전 생성 풀 중 `enemy_chaser` 12개를 활성 유지하고 사망 후 1.5초 간격으로 보충한다. 추적체 몸통 조각 드롭 가설은 25%이며 Step 13 실제 웨이브 적용 또는 플레이테스트 결론 없이 100개 일괄 활성/15%로 되돌리지 않는다. 포위 폭발·G0 회귀는 `OSExplosionMathEditModeTests`, `OSExplosionControllerPlayModeTests`, `OSStep10ScenePlayModeTests`, 보충 스폰 회귀는 `OSStep06ScenePlayModeTests`로 확인한다.
