...
- Step 15.8 자연 전개 계약을 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.8 Natural Body Unfold`를 사용한다. 대시 종료 시 `OSBodyChain` 경로를 도착한 머리 위치로 재고정해 정지 중에는 활성 몸통이 뭉친 상태를 유지하고, 이후 평상 이동 경로와 세그먼트 간격만큼 순차적으로 다시 펴지게 한다. 별도 시간 기반 재전개는 사용하지 않는다. 대시는 `OSPickup`이나 수집 가능 표식을 생성하지 않고 기존 픽업도 변경하지 않는다. 제거된 재전개 업그레이드는 `dash_recharge` 고정 쿨다운 -0.2초/단계로 대체하며 최종 하한 0.5초를 지킨다. 회귀는 `OSBodyDashMathEditModeTests`, `OSBodyDashControllerPlayModeTests`, `OSLevelUpCoreEditModeTests`, `OSLevelUpPlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_8/WebGL/`에 둔다.
