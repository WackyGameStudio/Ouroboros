...
- Step 03 세션 기반을 다시 연결하거나 누락을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 03 Session Foundation`을 사용한다. 세션 상태와 `Time.timeScale`은 `OSGameSessionController`만 소유하고, Player/UI Action Map 전환과 입력 구독 생명주기는 `OSInputRouter`만 소유하며 두 Map을 동시에 활성화하지 않는다. `UI/Submit.performed`에서는 요청만 기록하고 같은 입력 업데이트의 `InputSystem.onAfterUpdate`에서 전달하며, Action 콜백 안에서 활성 UI Map을 동기 Disable/Reset하지 않는다.
...
- 카메라는 `OSCameraFollower`가 머리만 직교 크기 6.5의 고정 줌으로 추종하며 몸통 길이에 따라 줌을 바꾸지 않는다. 16:9 월드 가시 영역은 약 23.1×13.0이고 Screen Space HUD 크기는 변경하지 않는다. 이동 회귀는 `OSPlayerMovementEditModeTests`, `OSPlayerMovementPlayModeTests`, `OSStep04ScenePlayModeTests`로 확인한다.
...
- 머리 HP 피해는 `OSPlayerHealth`가 피격 무적 0.6초를 관리하고 사망 즉시 세션을 중단한다. 몸통 절단은 `OSBodyChain.TryCutFrom`을 통해 0.35초 전체 절단 방지를 적용하며 제거 View는 꼬리→머리 역순으로 비활성화하고 외부 이벤트의 안정 ID는 원래 머리→꼬리 순서를 유지한다. 피해·절단 회귀는 `OSCombatEventBufferEditModeTests`, `OSPlayerDamagePlayModeTests`, `OSStep09ScenePlayModeTests`, 적 다중 접촉 공격 ID는 `OSEnemyLifecyclePlayModeTests`로 확인한다.
- Step 10 Space 능력 기반을 다시 연결하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 10 Body Convergence Dash`를 사용한다. 현재 규칙은 Step 15.8까지 갱신된 `OSBodyDashController` 계약을 따르며, 과거 Space 포위 폭발의 적 피해·몸통 소비·무적·예약 규칙과 `OSExplosion*` 타입을 복구하지 않는다. Step 16.1의 B Bomb은 별도 입력·상태·실제 원 주행을 사용하는 `OSBomb*` 계약이며 Space 대시에 다시 연결하지 않는다. G0의 `OSEnemyDebugSpawner` 12마리 보충은 Step 13 적용 뒤 비활성 상태로 보존하고, 추적체 몸통 조각 드롭 가설 25%는 플레이테스트 결론 없이 변경하지 않는다.
- Step 11 역할 몸통 기반을 다시 연결하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 11 Body Roles`를 사용한다. `OSBodyRoleRegistry`만 몸통 추가·절단 이벤트를 역할별 고정 배열 목록과 안정 ID 참조로 반영하며 몸통 수축 대시는 역할, 개수, 안정 ID, 화력 상태를 바꾸지 않는다. Attack은 피해 10으로 기존 `head_projectile` 120개 풀을 공유하고, Control은 피해 0 전용 `body_control_projectile` 64개 사전 생성 풀을 사용하며 두 역할 모두 대여 성공 때만 해당 세그먼트의 발사 주기를 소비한다.
- Laser는 길이 14·폭 0.35의 시작점·방향 스냅샷과 0.2초 예고 후 같은 사각형으로 `EnemyHurtbox` Collider를 비할당 `Physics2D.OverlapBox` 조회한다. `LineRenderer.widthMultiplier`는 1로 고정하고 시작·끝 폭을 같은 스냅샷 전체 폭으로 설정해 표시 굵기와 피격 굵기를 일치시킨다. 범위에 조금이라도 겹친 모든 등록 적에게 한 빔당 1회 피해를 주고 같은 적의 여러 Collider는 Runtime ID로 중복 제거하며, 원본 세그먼트가 예고 중 제거되면 취소한다. Control 재적용은 이동·공격 정지의 남은 시간과 새 지속 시간 중 큰 값만 유지한다. Shield는 명백히 무효인 머리 무적·몸통 절단 방지 피격에는 소비하지 않으며 유효 피격만 `OSPlayerCombatResolver` 앞 단계에서 방어한다. 적 접촉·적 투사체의 Shield 범위 판정점은 대상 중심이 아니라 충돌 `Collider2D.ClosestPoint(공격원 위치)`이며 콜라이더가 없을 때만 대상 중심으로 대체한다. 역할 제거 시 신규 효과와 내부 충전·예고 상태는 폐기하되 이미 발사된 Attack/Control 투사체는 유지한다. 역할 회귀는 `OSBodyRoleMathEditModeTests`, `OSBodyRolesPlayModeTests`, `OSStep11ScenePlayModeTests`로 확인한다.
...
- `OSLevelUpController`가 17종 업그레이드 단계와 불변 Modifier 스냅샷을 소유해 머리 무기, 몸통 성장·길이 화력, 역할 4종, 대시, Bomb, HP·이동·회복, 자석·경험치, 정예 우선 표적에 배포한다. 발사 주기 0.15초, 기본 조각 요구량 6·성장 효율 단계 5/4, 대시 거리·배율 쿨다운·고정 쿨다운 감소의 최종 0.5초 하한, Bomb 재사용 최종 5초 하한, 이동 속도 7.5/s 하한·상한을 유지하고 SO 원본은 수정하지 않는다. 경험치·몸통 조각·회복·절단 몸통은 기존 `body_fragment_pickup` 256개 풀을 `OSPickupType`별 효과·병합·색상으로 공유하며 `SeveredBody`는 원래 역할과 수량을 추가로 보존한다. 레벨업 회귀는 `OSLevelUpCoreEditModeTests`, `OSLevelUpPlayModeTests`, `OSStep12ScenePlayModeTests`로 확인한다.
- Step 13 시간 웨이브를 다시 연결하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 13 Timed Waves`를 사용한다. `OSWaveDirector`만 세션별 `OSWaveScheduleRuntime` 복사본, 런 시드 13013, 전투 시간, 누적 스폰 티켓과 3·6분 정예/9분 경고를 소유하며 선택 중에는 진행하지 않는다. 일반 적은 카메라 밖·머리 거리 7 이상·월드 경계 안·`WorldBlocker` 밖 후보를 최대 8회 검사하고, 실패 또는 활성 일반 적 180 상한에서는 티켓을 다음 틱으로 넘긴다. HP는 `1.12^(elapsed/60)`, 생성률은 Step 15.10의 0~3분 `1.15^minute`·이후 `1.15^3 × 1.02^(minute-3)`에 Step 16.2의 `OSWaveScheduleData.spawnDensityMultiplier = 1.2`를 곱해 적용하며 10:00 보스 이벤트는 Step 14의 `OSBossEncounterController`로 전달한다.
...
- Step 14 보스·결과 기반을 다시 연결하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 14 Boss And Results`를 사용한다. `boss_swarm_core`는 전용 1칸 풀에서 10:00에 1회 생성하며 일반 적 180 상한과 별도다. `OSBossController`는 HP 6,000, 보호막 600, HP 70%/35% 페이즈, 150초 제한, 부채꼴 5/7발·일반 적 6/8마리 소환·충돌 안전 흡인·보호막 패턴을 소유하고 한 시점의 고위험 예고를 1개로 제한한다. 선택 중에는 패턴과 제한 시간을 정지하고 제어는 이동만 제한하며 보스 캐스팅을 취소하지 않는다.
- `OSBossEncounterController`는 플레이어 피해 처리 뒤 보스 사망을 확정해 같은 틱의 플레이어·보스 사망은 플레이어 실패로 처리하며, 보스 처치와 150초 시간 초과를 서로 다른 결과로 세션에 전달한다. `OSRunSummaryController`는 몸통·처치·머리 피해·역할·업그레이드·대시 사용/수축 몸통·Bomb 사용/소비 몸통/고유 적 명중·런 시드/데이터 버전을 누적해 종료 시 불변 요약으로 만들고 `OSResultPanel`은 결과 중 전투 입력을 막은 채 재시작과 메인 메뉴를 제공한다. 보스·결과 회귀는 `OSBossMathEditModeTests`, `OSStep14ScenePlayModeTests`와 전체 EditMode/PlayMode 회귀로 확인한다.
- Step 15 HUD·판독성 기반을 다시 연결하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15 Readability`를 사용한다. `OSCombatHudPresenter`는 규칙 상태를 수정하지 않고 이벤트를 dirty 표시 모델로 모아 `LateUpdate`에서 프레임당 최대 1회 반영한다. 플레이어 체력은 `OSPlayerHealth` 한 값을 `HP 현재/최대`로 표시하며 구형 정적 `HP`와 플레이어 `CORE` 중복 표기를 노출하지 않는다. 몸통 HUD에는 현재 수만 표시하고 기술 가드 64를 최대치·분모로 노출하지 않으며 Shield/Attack/Laser/Control은 색과 `[O][>][=][+]` 문양을 함께 사용한다.
- 머리 코어, 적·보스 위험, 레이저, Bomb 원/폭발 채움, 절단, 제어 원호, 실드의 Sorting Order는 각각 220, 200, 198, 190/189, 185, 182, 175다. 몸통 수축 대시는 기존 몸통 View를 보간하고 별도 폭발 예약 View를 만들지 않는다. `OSTutorialProgress`는 첫 세션 조건 전이와 unscaled 시간을 순수 상태로 소유하고, `OSCombatFeedbackPresenter`는 머리 피해와 절단 위치·꼬리 방향·상실 역할을 표시만 하며 규칙 결과에 관여하지 않는다. HUD·튜토리얼·고밀도 판독성 회귀는 `OSTutorialProgressEditModeTests`, `OSStep15ScenePlayModeTests`와 전체 EditMode/PlayMode 회귀로 확인한다.
- Step 15.7 몸통 수축 대시 기반을 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.7 Body Convergence Dash`를 사용한다. `BodyDash` 입력은 Space를 유지하고 전투 중 0.5초 동안 머리를 스냅샷 진행 방향으로 기본 4.5 이동시키며 `OSPlayerController`의 기존 비할당 Kinematic Cast와 월드 경계 제한을 그대로 적용한다. 사용은 몸통 0칸에서도 가능하며 적 피해, 피격 무적, 몸통 제거·역할 변경을 만들지 않는다. 기본 재사용 대기시간은 발동 시점부터 2초다.
- Step 15.8 자연 전개 계약을 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.8 Natural Body Unfold`를 사용한다. 대시 종료 시 `OSBodyChain` 경로를 도착한 머리 위치로 재고정해 정지 중에는 활성 몸통이 뭉친 상태를 유지하고, 이후 평상 이동 경로와 세그먼트 간격만큼 순차적으로 다시 펴지게 한다. 별도 시간 기반 재전개는 사용하지 않는다. 대시는 새 `OSPickup`이나 수집 가능 표식을 생성하지 않는다. 제거된 재전개 업그레이드는 `dash_recharge` 고정 쿨다운 -0.2초/단계로 대체하며 최종 하한 0.5초를 지킨다. 회귀는 `OSBodyDashMathEditModeTests`, `OSBodyDashControllerPlayModeTests`, `OSLevelUpCoreEditModeTests`, `OSLevelUpPlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_8/WebGL/`에 둔다.
- Step 15.9의 초반 기본 생성률 20% 완화는 Step 15.10에서 폐기됐다. 과거 메뉴 `Ouroboros/Setup/Apply Step 15.9 Slower Wave Pacing`도 최신 Step 15.10 계약을 적용해 회귀를 만들지 않는다.
- Step 15.10 후반 웨이브 생성 속도를 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.10 Late Wave Pacing`을 사용한다. 일반 생성 구간 `spawnRate`는 원래 값 `0.50/0.75/1.00/1.20/1.40/1.60/1.80/2.00/2.00/2.00`이며, 배율은 0~3분 `1.15^minute`, 이후 `1.15^3 × 1.02^(minute-3)`이다. 적 가중치, 목표 활성 수, 3·6분 정예, 9분 경고, 10분 보스, 180 하드캡과 안전 위치·티켓 보류 규칙은 바꾸지 않는다. 회귀는 `OSWaveScheduleEditModeTests`, `OSStep13ScenePlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_10/WebGL/`에 둔다.
- Step 15.11 게임 카메라 시야를 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.11 Wider Camera View`를 사용한다. `20_Game`의 직교 카메라 크기는 6.5이고 머리 중심 고정 추종·무회전·경계 여백과 화면 밖 스폰 규칙을 유지한다. 회귀는 `OSStep04ScenePlayModeTests`, `OSStep13ScenePlayModeTests`와 전체 PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_11/WebGL/`에 둔다.
- Step 15.12 UI Submit 안전 전환은 `OSInputRouter`가 Submit을 Action 콜백 안에서 즉시 전달하지 않고 `InputSystem.onAfterUpdate`에서 입력 업데이트당 최대 1회 전달하게 한다. 비활성화 때 보류 요청과 정적 이벤트 구독을 정리하고, Dead→Result→재시작 중 이후 Submit 구독자의 Control/Device 안전성은 `OSInputRouterPlayModeTests`로 확인한다. WebGL 산출물은 `Builds/Step15_12/WebGL/`에 둔다.
- Step 15.13 머리 진행 방향 비주얼을 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.13 Head Facing Visual`을 사용한다. `OSPlayerHeadVisual`은 물리 머리 루트가 아니라 `coreVisual`만 `LastDirection`으로 회전시키고 정지 시 마지막 방향을 유지한다. 현재 `Head.png`의 눈 정면축은 로컬 +Y(`spriteForwardDegrees=90`)이며 리소스 교체 시 이 값만 새 원본 축에 맞춘다. 회귀는 `OSPlayerMovementPlayModeTests`, `OSStep04ScenePlayModeTests`와 전체 PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_13/WebGL/`에 둔다.
- Step 15.14 꼬리 회수 전개·HP HUD를 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.14 Tail Reclaim And HP HUD`를 사용한다. `OSBodyChain.AppendReclaimedSegments`는 한 회수 묶음의 공통 누적 경로 기준점을 수집 순간의 기존 마지막 꼬리로 잡아 신규 View 전량을 겹치고, 머리 실제 이동 거리 1칸마다 앞쪽부터 정상 간격에 합류시킨다. 기준점과 활성 여부는 몸통 풀과 같은 사전 할당 배열에 보존하며 회수 중 컬렉션을 만들지 않는다. 통합 HUD는 `HealthChanged` 기반 `HP`만 표시하고 `20_Game`의 정적 `HP 100/100` 텍스트를 비활성화한다. 회귀는 `OSBodyGrowthPlayModeTests`, `OSStep15ScenePlayModeTests`와 전체 PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_14/WebGL/`에 둔다.
- Step 15.15 대시 픽업 흡입을 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.15 Dash Pickup Suction`을 사용한다. `OSPlayerController`가 실제 Cast·슬라이드 대시 이동 구간을 발행하면 `OSPickupSpawner`는 고정 활성 배열을 비할당 순회해 현재 유효 자석 반경에 걸린 모든 기존 픽업을 흡입 상태로 고정한다. 픽업은 즉시 소멸하지 않고 24 유닛/초로 머리를 추적하며 대시 종료 뒤 반경 밖에서도 추적을 유지하고 `PickupCollector` 접촉 때만 기존 효과·부분 회수 규칙을 적용한다. 회귀는 `OSBodyDashControllerPlayModeTests`와 전체 PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_15/WebGL/`에 둔다.
- Step 15.16 통합 플레이어 HP 바를 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.16 Player HP Bar`를 사용한다. `OSCombatHudPresenter`가 숫자와 가로형 HP 바를 모두 `OSPlayerHealth.HealthChanged` 기반 dirty 표시 모델로 갱신하며, 바는 `Clamp01(CurrentHealth / MaxHealth)` 비율을 왼쪽 기준으로 표시한다. 별도 체력 상태를 만들지 않고 구형 `Canvas/CombatHUD/PlayerHealthHUD`는 비활성 상태를 유지한다. 회귀는 `OSStep15ScenePlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_16/WebGL/`에 둔다.
- Step 15.17 전투 명료성 마감을 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.17 Combat Clarity`를 사용한다. 보스 제한 시간은 선택 중 정지하는 150초이고 결과에는 `150s TIMEOUT`을 표시한다. 공용 픽업 풀의 몸통 조각·경험치·회복은 각각 청록 마름모·금색 원형·초록 십자 스프라이트로 구분하며 재대여 때 타입별 표시를 복구한다. Laser 표시선은 `widthMultiplier=1`과 판정 스냅샷 폭을 사용한다. 회귀는 `OSBodyRolesPlayModeTests`, `OSStep14ScenePlayModeTests`, `OSStep15PickupScatterPlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_17/WebGL/`에 둔다.
- Step 15.18 플레이어 HP 바 시각 갱신 복구를 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.18 HP Bar Visual Fix`를 사용한다. `HealthBarFill`은 내장 `UISprite`를 사용하는 `Filled Image`여야 하며 `fillAmount`와 Canvas 실제 렌더 메시 폭이 `CurrentHealth / MaxHealth`와 함께 감소해야 한다. 막대는 좌상단 HP 숫자 바로 아래의 상단 앵커에 두고 구형 `PlayerHealthHUD`는 비활성 상태를 유지한다. 회귀는 `OSStep15ScenePlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_18/WebGL/`에 둔다.
- Step 15.19 레벨업 보상 설명 개선을 다시 적용할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 15.19 Reward Clarity`를 사용한다. 보상 카드는 계열·이름·쉬운 효과 문장·`CURRENT → AFTER` 실제 수치·현재/다음/최대 단계 순으로 표시하며, 효과 문장에는 대상과 단계당 변화량을 함께 쓴다. 비교 수치는 밸런스 SO와 하한·상한을 반영하고 피해·주기·개수·거리·HP 등의 의미와 단위를 생략하지 않는다. 회귀는 `OSLevelUpCoreEditModeTests`, `OSLevelUpPlayModeTests`, `OSStep12ScenePlayModeTests`와 전체 EditMode/PlayMode로 확인하며 WebGL 산출물은 `Builds/Step15_19/WebGL/`에 둔다.
- Step 16은 Step 15.x와 같은 사용자 플레이 피드백 반복 개선 구간이다. 피드백 한 묶음을 독립 검증 가능한 최소 `Step 16.x`로 등록하고 관찰 문제·재현 조건·기대 체감·수용 기준을 먼저 기록한다. 각 반복은 설계 정합성 확인, 구현, Unity 컴파일·Console 확인, 위험에 맞는 집중/전체 테스트, WebGL Development 빌드·HTTP 브라우저 수동 확인, 구현 현황 갱신, Step 전용 커밋·원격 푸시까지 끝낸 뒤 다음 피드백으로 넘어간다. 사용자가 합의된 피드백 반영 완료와 게임성 기준선 확정을 확인해야 Step 17로 진행한다.
- Step 16.1 Bomb 기반을 다시 적용할 때 과거 메뉴 `Ouroboros/Setup/Apply Step 16.1 Bomb`도 현재 Step 16.2 계약을 적용해 반지름·피해·장애물·입력 규칙을 되돌리지 않는다.
- Step 16.2 Bomb 리듬·밀도 피드백을 다시 적용하거나 누락 설정을 보정할 때 Unity 메뉴 `Ouroboros/Setup/Apply Step 16.2 Bomb Rhythm And Density`를 사용한다. `B` 홀드 중 현재 머리 위치·진행 방향·몸통 배치에 따른 완성 원을 매 프레임 표시하고 릴리스 때만 발동한다. 활성 몸통 10칸 이상에서 `floor(N×0.10)`을 꼬리부터 소비하고 반지름은 소비 후 길이 기반 값의 1.5배다. 회전 방향은 몸통이 많은 쪽의 반대이며 1초 원 주행은 사인 ease-in-out 완급을 사용하고 `WorldBlocker`·월드 경계를 무시한 뒤 원점으로 복귀한다. 전 구간 머리·몸통 무적, 0.5초 원 경로 집결, 기본 10초 재사용을 유지한다. 폭발은 발동 직전 몸통 수×몸통당 10×강화 배율이며 `bomb_damage`는 몸통당 +20%/단계 최대 3, `bomb_cooldown`은 -1초/단계 최대 3·최종 하한 5초다. 일반 적 스폰 예산은 SO의 1.2 밀도 배율을 추가 적용하고 180 하드캡은 유지한다. 회귀는 `OSBombMathEditModeTests`, `OSBombControllerPlayModeTests`, `OSStep16BombScenePlayModeTests`, `OSInputRouterPlayModeTests`, `OSWaveScheduleEditModeTests`와 전체 회귀로 확인하며 WebGL 산출물은 `Builds/Step16_2/WebGL/`에 둔다.
- Step 14 이후 구현 순서는 Step 15 HUD·튜토리얼·전투 판독성 → Step 15.1 전장 충돌 → Step 15.2 실드·성장 밸런스 → Step 15.3 픽업 분산 → Step 15.4 레이저 Collider 범위 → Step 15.5 몸통 화력·절단 회수 → Step 15.6 절단 몸통 소형 비주얼 → Step 15.7 몸통 수축 대시 → Step 15.8 자연 전개 → Step 15.9 적 웨이브 생성 속도 완화 → Step 15.10 후반 웨이브 생성 속도 완화 → Step 15.11 게임 카메라 시야 확대 → Step 15.12 UI Submit 콜백 안전 전환 → Step 15.13 머리 진행 방향 비주얼 → Step 15.14 꼬리 회수 전개·HP HUD → Step 15.15 대시 픽업 흡입 → Step 15.16 통합 플레이어 HP 바 → Step 15.17 전투 명료성 마감 → Step 15.18 플레이어 HP 바 시각 갱신 복구 → Step 15.19 레벨업 보상 설명 개선 → Step 16 사용자 플레이 피드백 반복 개선 → Step 17 성능 최적화·WebGL → Step 18 사운드·VFX·연출 → Step 19 자동 테스트·빌드 게이트로 고정한다. Step 17에서 코어 전투의 성능 기준선과 연출 예산을 먼저 확정하고, Step 18은 그 예산 안에서 구현한 뒤 같은 피크 장면과 WebGL 메모리를 재측정한다.
